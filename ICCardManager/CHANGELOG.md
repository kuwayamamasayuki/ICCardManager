...
- Issue #1744 **BOM 無し Shift_JIS の CSV をインポートすると日本語が文字化けしたまま DB へ保存される**問題を修正した。`CsvImportService.ReadCsvFileAsync` は BOM を検出しつつも、BOM が無い場合は `Encoding.UTF8`（不正バイトを U+FFFD へ**黙って置換する**フォールバック）固定で復号していた。日本語版 Excel でエクスポート CSV を開いて「CSV（コンマ区切り）」で保存し直すと BOM が失われ ANSI（Shift_JIS）になるため、マニュアル §5.6.5 が推奨する「エクスポート CSV をテンプレートに編集する」運用が、そのまま文字化けの経路になっていた。
  ...
  - **判別できないファイルはインポートを中断する**（`FileOperationException.UndecidableEncoding` / FILE008）。文言は「何が／なぜ／どうすれば」の 3 要素で、Excel の保存形式名「CSV UTF-8（コンマ区切り）(\*.csv)」まで示す。利用履歴の 2 経路は共通ハンドラーではなく**独自の catch 連鎖**を持つため、そちらにも `FileOperationException` の捕捉を足した（漏らすと `catch (Exception)` に落ちて「予期しないエラー: …」と生の例外メッセージが出る／#1614）。
  - **Shift_JIS として読んだことを `LogInformation` で残す**。文字化けの相談を受けたときに「どの文字コードとして読んだか」がログに無いと切り分けられない（`LogDebug` は本番のログレベル設定では出力されない／#1716）。UTF-8 と判別した場合は出力しない。
  - 検証: `TextEncodingDetectorTests`（18件）を新設、`CsvImportServiceTests` に 10 件追加（+28）。回帰テストは実際に cp932 でバイト列を書き出して「山田太郎」が復元されること・置換文字を含まないことを表明し、修正前は `�R�c���Y` として `staff.Name` へ渡っていたことを確認済み。UTF-8／Shift_JIS の**両方向の誤判定**（UTF-8 が Shift_JIS と判定されない／その逆）を別テストで固定する。
  - 05_クラス設計書 §5.7.2／07_テスト設計書 §1.1a（単体 4,698→4,726・合計 4,724→4,752 件）・§2.17 UT-TEXTENC / UT-CSVIMP-ENC／管理者マニュアル §5.6.5・§6.4・§10.3 を同期更新（#1744）
