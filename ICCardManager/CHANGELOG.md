...
- Issue #1737 **起動時の自動バックアップが後続の保守タスクと同一 DB 接続上で並走し、バックアップ成功の記録が消える**問題を修正した。
  ...
  - **対処（settings 書き込みの保護）**: `SettingsRepository` の書き込み（`SetAsync` / `TryAcquireMonthlyVacuumLockAsync`）をセマフォ保護下（`BeginTransactionAsync`）へ移した。外側にトランザクションスコープがある場合は接続だけを借りて暗黙参加する2分岐とする — `SaveAppSettingsAsync` は1スコープ内で `SetAsync` を10回以上呼ぶため、無条件に開くと `SemaphoreSlim(1,1)` の自己デッドロックになる（#1575）。他リポジトリ（Card / Ledger / Staff）の同型の書き込み17箇所は、いずれもユーザーの単一操作を起点とし状態機械で直列化されるため対象外とした。
  - **対処（VACUUM 失敗時のログ）**: `DbContext.Vacuum()` が Busy / Locked 以外の SQLite エラーも false（当月スキップ）へ畳み、`ResultCode` を Warning ログに残すようにした。呼び出し元は CAS ロックを消費してから VACUUM へ来るため、例外を投げ返してもロックは戻らず当月は再試行されない。従来の痕跡は `#if DEBUG` の `Debug.WriteLine` のみで本番ログに残らなかった（#1716）。失敗理由はモードにより異なるため原因は断定せず ResultCode の提示に留める。
  - 検証: `StartupTaskRunnerTests`（10件）・`SettingsRepositorySetConcurrencyTests`（4件）・`DbContextVacuumTests`（3件）を新設（+17）。**並列化の回帰は「順序」ではなく同時実行数で表明する**（修正前の実測は 2）。外側の保持側はテスト用に `BeginTransactionAsync` で代用せず、実際の `CleanupOldDataInternal` と同じ同期リース＋トランザクションで再現する（代用すると `HasActiveTransactionScope` が立ち、検証したい経路を通らない）。
  - 05_クラス設計書 §5.5c・§3.3・§6.1／07_テスト設計書 §1.1a（単体 4,494→4,511・合計 4,520→4,537 件）・§2 UT-078 を同期更新（#1737）
