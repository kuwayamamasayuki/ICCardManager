...
- Issue #1784 **インポート結果ダイアログの背後にプログレスバーが残存する**問題を修正した（Issue #1383 のインポート版）。
  ...
  - 検証: `DataExportImportViewModelTests` に5件追加（+5）。成功（`ShowInformation`）・失敗（`ShowError`）・部分成功（`ShowWarning`）・**成功＋監査ログ記録失敗（`ShowWarning`）**・例外（`ShowError`）の5つの表示地点それぞれについて、`Callback` で**呼び出し時点**の `IsBusy` を捕捉して `false` であることを固定した。`IsBusy` の最終値は修正前でも `false`（`using` が必ず `Dispose` する）のため最終値の検査では検出できない。修正前の実装で5件とも `but found True` で失敗することを確認済み。
  - 03_画面設計書 §3.10（結果ダイアログと処理中オーバーレイを重ねない）・§4.4／05_クラス設計書 §5.17b（結果ダイアログは `BeginBusy` スコープの外で表示する）／07_テスト設計書 §1.1a（単体 4,608→4,613・合計 4,634→4,639 件）・§2 UT-028 No.40〜44 を同期更新（#1784）
