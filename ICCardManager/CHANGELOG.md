...
**バグ修正**
- Issue #1757 **カード編集で管理番号が重複すると致命的エラーダイアログになる**問題を修正した。`CardRepository.InsertAsyncInternal` は UNIQUE 制約（`idx_card_type_number_active`）違反を捕捉して `DuplicateCardNumberException` へ変換し、カード登録では「管理番号 N-001 は同じ種別で既に使用されています。別の番号を指定してください。」という復旧可能な案内が出る。しかし `UpdateAsyncInternal` には同等の catch が無く、編集経路には管理番号の重複チェックも存在しない（`ValidationService.ValidateCardNumber` は書式・長さのみ、`GetByIdmAsync` による重複チェックは新規カード分岐でしか走らない）。このため既存カードの管理番号を、同一種別の別カードが使っている値へ変更して保存すると、生の `SQLiteException` が `App.OnDispatcherUnhandledException` まで抜け、**`SYS00x` エラーコードとスタックトレース付きの致命的エラーダイアログ**が出て編集内容も失われていた。**同じ操作が登録では親切に案内され、編集ではクラッシュ相当**という非対称な状態だった
  ...
  - **CSVインポート経路の退行も同時に塞いだ**。`UpdateAsync` が新たに `DuplicateCardNumberException` を投げるようになると、`CsvImportService.Card.cs` の `catch (SQLiteException)` に掛からなくなり `DatabaseException` へのラップを経ずに再スローされる。そのままだと `ToUserFacingErrorMessage` の `default` 分岐に落ちて「**予期しないエラーが発生しました**: …」という誤った枕詞が付き、かつ**何行目が悪いのか分からない**（他のインポートエラーは行番号付き）。取り込みループで本例外を捕捉し、行番号付きの `CsvImportError` として報告するようにした。**登録経路（#1106 から例外化済み）にも同じ穴が既にあり、こちらも同時に解消している**
  - 検証: `CardNumberUniqueConstraintTests` +3件（重複更新で例外／削除済みカードの番号へは更新可／番号を変えない更新は成功）、`CardManageViewModelTests` +1件（致命的にならず行動指示付きメッセージ＋編集モード維持）、`CsvImportServiceTests` +2件（更新・登録の両経路で行番号付きエラー）の計 +6件。うち重複を検出すべき 4 件は修正前のコードで赤になることを実測済み（正常系ガード 2 件は修正前後とも緑になる設計）
