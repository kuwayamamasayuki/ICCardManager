...
- 管理者マニュアル §4「職員管理」のダイアログスクリーンショットを追加。従来は §4.1 で職員管理画面（`staff.png`）1 枚のみで、§4.2「職員の登録」「職員情報の編集」「職員証の再登録」で開かれる `StaffManageDialog` の実画面が無く、新任管理者が初めて職員登録を行う際の入力項目の見え方を事前に確認できなかった（研修資料としても活用しにくい状態）。(1) §4.2「職員の登録」の手順 1 直下に「タッチ前／タッチ後」を 2 列テーブルで並置（`staff_register_before_touch.png` / `staff_register_after_touch.png`）し、職員証タッチによって IDm 欄が自動入力される流れを視覚化、(2) §4.3「職員情報の編集」の手順 2 直下に編集ダイアログ（`staff_edit_dialog.png`）、(3) §4.5「職員証の再登録」の手順 2 直下にも同 `staff_edit_dialog.png` を再掲。`StaffManageDialog` は編集モード（`IsEditing=true`）で「職員証をタッチしてください」プロンプト（XAML L162 Setter）を自動表示するため §4.3 と §4.5 で同一画像を参照可能だが、各見出しから独立して読めるよう重複参照を許容（ユーザーマニュアル §3 の `touch_staff_card.svg` 多重参照と同方針）。撮影手順は `tools/TakeScreenshots.ps1` の Issue #1414 エントリ（行 455〜473）に既に整備済み（chore commit 2bb1bb2 で先行追加）。本コミットでは編集ダイアログの Instructions に §4.3 / §4.5 双方で同一画像を参照する旨を追記し、撮影者が「再登録時の画面も別撮影が必要では?」と迷う運用ノイズを排除。画像本体はユーザー側で別コミットで追加（Issue #1411 / #1412 と同じ運用）。本コミットはマニュアル本文・撮影スクリプト指示・CHANGELOG のみ（#1414）
