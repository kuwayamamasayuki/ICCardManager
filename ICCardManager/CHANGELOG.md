...
- Issue #1811 **警告メッセージの提示の欠陥 2 件**（カードリーダーエラー警告の無限蓄積・バス停名入力の警告が表示前に上書きされる）を修正した。片方は消えず、もう片方は消えるのが早すぎた。
  ...
  - 検証: `MainViewModelTests` +4（集約と回数・ユーザー向け文言・クリックでの消去と回数の振り出し・他種別を巻き添えにしない）、`BusStopInputViewModelTests` +4（確認ダイアログに全警告が載る・「いいえ」で保存せず警告が残る・警告なしは確認を挟まない・類似警告の上限と要約）、`MainWindowWarningAreaLayoutTests` +1（クリックヒントを種別トリガーで設定）、`BusStopInputDialogStatusAreaLayoutTests` を新設（ステータス欄の折り返し）。既存の #1807 テスト 2 件は「警告が 1 件」で二重購読を検出していたが集約後は常に 1 件になるため、`OccurrenceCount == 1` を見る形へ改めた（検出力を維持）。新規テストは修正前のコードに当てて 7 件が赤になることを確認済み。
  - 03_画面設計書 §3.8／04_機能設計書 §7.4／05_クラス設計書 §12.2／07_テスト設計書 §1.1a（単体 5,069→5,079・合計 5,095→5,105 件）・UT-017a4・UT-WARN-KEEP・UT-065a／ユーザーマニュアル §3.4・§4.3／`.claude/rules/development-conventions.md`（自動で閉じる画面のステータス欄に判断を要する警告を出さない）を同期更新（#1811）
