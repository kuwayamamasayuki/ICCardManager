...
- Issue #1761 **カード管理・職員管理で編集中に一覧の選択が外れると、削除・払い戻しが「予期しないエラー」で中断する**問題と、**競合の案内が一覧に存在しない値で対象を名指しする**問題を修正した。編集フォームは `IsEditing` にのみ連動して表示される一方、一覧の選択は `SelectedItem`（TwoWay バインド）で ViewModel へ書き戻されるため、**選択行の Ctrl+クリック**（一覧は単一選択でも選択解除できる）や**一覧の再読み込み**によって選択だけが外れる。フォームは開いたまま・入力内容も残るため、利用者には「何も起きていない」ように見える。
  - **削除の認証（職員証タッチ）待ち・払い戻しの残高取得待ちの間に選択が外れると、確認ダイアログの組み立てで `NullReferenceException` になっていた**。非同期コマンドから例外が抜けるため「予期しないエラーが発生しました」の致命的エラーダイアログになり、**行の選択が外れたことが原因だと利用者には分からない**。識別情報（IDm・カード種別・管理番号）を**最初の `await` より前**に確定させ、以降はその値だけを使うようにした。削除・払い戻しの対象は「ボタンを押した時点で選択されていた行」であり、その後の選択状態には依存しない
  ...
  - **カード管理と職員管理の両方を是正した**。`IsEditing` を持つ ViewModel はこの 2 つだけであることを確認したうえで横断で洗った（`.claude/rules/development-conventions.md`「規約を新設したら同種の既存箇所を横断で洗う」）
  - 検証: `CardManageViewModelTests` +7件・`StaffManageViewModelTests` +5件（+12）。**修正前のコードに当てて 5 件が赤になること**（`NullReferenceException` 2 件・誤った名指し 3 件）を実測して確認した。選択解除のタイミングはモックの `Callback` で認証・残高取得の待機点に合わせ、本番で利用者が待機中に操作した状況を再現する。残りの 7 件は修正前も緑だが、これは Issue #1726 で `SaveAsync` の逆参照が既に解消されていたためで、**不変条件を将来の退行から守る固定**として置く
