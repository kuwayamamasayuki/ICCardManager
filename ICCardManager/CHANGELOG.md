...
**バグ修正**
- 新規職員登録ダイアログで職員証をタッチした直後、氏名入力欄へキーボードフォーカスが移らずユーザーがマウスで氏名欄をクリックしないと文字入力できなかった問題を修正。`StaffManageDialog.xaml` には氏名 TextBox への明示的なフォーカス指定（`x:Name`／`FocusManager.FocusedElement`）が無く、`StaffManageViewModel.StartNewStaffWithIdmAsync` が `IsWaitingForCard = false` を立てた後も View 側にフォーカス指示が伝わらない構造だった。最終的に WPF ネイティブの **`Window` タグへの `FocusManager.FocusedElement="{Binding ElementName=NameTextBox}"` 指定** を主軸に採用し、WPF 内部のフォーカス確定タイミング（Window アクティベート時）に直接乗ることで Loaded／ContentRendered／Activated の発火順や Dispatcher 優先度競合に依存しない実装にした。`IsEditing=false` で NameTextBox が Visibility=Collapsed の場合は WPF が自動的にフォーカス対象外として扱うため、職員管理を素で開いた場合の挙動への副作用なし。加えて Issue 推奨の「案 1（MVVM 原則準拠）」も併用し、(1) `StaffManageViewModel` に `event EventHandler? RequestNameFocus` を追加して未登録職員証分岐の末尾で発火（既登録／削除済み分岐は `return true` でダイアログを閉じるため発火させない）、(2) `StaffManageDialog.xaml` の氏名 TextBox に `x:Name="NameTextBox"` を付与、(3) `StaffManageDialog` コードビハインドが「`RequestNameFocus` 受信」と「`ContentRendered` 発火」の **AND 条件** を満たした時のみ多重防御としてフォーカスを再確定する設計とした。当初実装の `DispatcherPriority.Input` → `ContextIdle` への変更や AND 条件化のみでは「Tab を押して初めて Caret が現れる」症状が解消せず、根本的に WPF 内部のフォーカス確定処理に介入する必要があったため XAML レベル指定を主軸に切替。編集パネルは `Visibility="{Binding IsEditing, ...}"` で制御されているため、起動時は `IsEditing=false` で Collapsed、`StartNewStaffWithIdmAsync` 内で `true` に切り替わるが切替直後はレイアウトパス未完了で `Focus()` が空振りする（当初実装の `DispatcherPriority.Input` でも次に試した `ContextIdle` でも、Loaded async 経路完了直後に走るため不十分で「Tab を押して初めて Caret が現れる」症状が継続）。`ContentRendered` を必須前提に AND 条件化することで Window の最終描画完了を保証し、確定タイミングで `Dispatcher.BeginInvoke(... ApplicationIdle)` 経由で `Activate()`（IsActive=false 時のフォールバック）→ `NameTextBox.UpdateLayout()`（Collapsed → Visible 切替後の同期レイアウト強制）→ `FocusManager.SetFocusedElement(this, NameTextBox)`（論理フォーカス）→ `Keyboard.Focus(NameTextBox)`（キーボードフォーカス）→ `NameTextBox.Focus()`（要素フォーカス）の 5 段がけで論理・キーボード・要素フォーカスを全層確定。`ApplicationIdle` は Dispatcher 最低優先度でフレームワーク内部の保留処理が全て落ち着いてから動くためフォーカス操作の競合を排除できる。回帰防止として `StaffManageViewModelTests` に 2 件追加し、(a) 未登録職員証で `RequestNameFocus` が 1 回発火し `shouldClose==false`、(b) 既登録職員証では発火せず `shouldClose==true`（ダイアログを閉じる分岐）であることを検証。実フォーカス確定は WPF Dispatcher 動作が必要なため手動テストで確認。`07_テスト設計書.md §2.42a` に手動テスト項目を追記（#1429）
