...
- Issue #1786 **ビルド警告 CS8618**（`SettingsRepositorySetConcurrencyTests`）を解消した。Release / Debug いずれのビルドでも 1 件出ていた唯一の警告で、コミット 564b9be0（Issue #1737 / PR #1776）で追加したテストヘルパー `CleanupSimulator` の `_worker` フィールドが未初期化だったことによる。
  ...
  - **再発防止として、抑制へ逃げる経路を規約テストで塞いだ**（`BuildWarningSuppressionConventionTests`）。CI にビルド警告のゲートが無く（`ci.yml` は警告を検査しない）、警告ゼロは「気付いた人が Issue を起票する」運用でしか担保されていないため、少なくとも「抑制で消した」痕跡が残るようにする。検査は ①3 プロジェクトから `NoWarn` の警告 ID を抽出できること（抽出の空振り検出） ②列挙されたすべての ID が同じ csproj のコメントで理由付けされていること ③テストプロジェクトが `CS8618` を抑制していないこと ④テストプロジェクトが `<Nullable>enable</Nullable>` を保っていること、の4点。④は「Nullable を無効化して Null 許容系の警告を一括で消す」最短の逃げ道を塞ぐもの。
  - **`NoWarn` の使用自体は禁じない**。テスト特有の表現（意図的な null 渡し等）に対する抑制は正当であり、禁じるのは理由の書かれていない抑制。あわせて理由コメントの無かった `DebugDataViewer.csproj` に説明を追記し、`ICCardManager.Tests.csproj` のコメントで圧縮表記していた ID（`CS8600/8602/…`）を完全な形（`CS8600, CS8602, …`）へ改めた（圧縮表記では突き合わせが誤検出するため）。
  - 検証: `BuildWarningSuppressionConventionTests`（4件）を新設（+4）。3 種の違反（`CS8618` と理由コメントの無い ID の追加／`Nullable` 値の改変／理由コメントの削除）を同時に注入し、抽出妥当性テストのみ green・残り3件が fail することを確認済み。Release / Debug 双方のソリューションビルドが 0 警告になることも実測した。
  - 07_テスト設計書 §1.1a（単体 4,617→4,621・合計 4,643→4,647 件）・§2 UT-079／開発者ガイド §4.7／`.claude/rules/development-conventions.md` を同期更新（#1786）
