...
- Issue #1692 **管理者ダッシュボード（F8）**を追加した（2026-07-10 ユーザビリティ総合検討 Phase 3）。メイン画面右の簡易警告はあるが、管理者が俯瞰したい情報（長期未返却・残高割れ・当月未出力帳票・カード稼働率）が1画面に集約されておらず、6年分蓄積される `ledger` も「記録と帳票」以外に活用されていなかった。運用状況の統制と「カード枚数は適正か」の判断材料を1画面にまとめる。
  ...
  - 検証: `ChartGeometryCalculatorTests`（51件）・`AdminDashboardServiceTests`（48件）・`ChartScaleTests`（47件）・`LedgerRepositoryAggregationTests`（36件）・`AdminDashboardViewModelTests`（33件）・`CardUtilizationCalculatorTests`（29件）・`AdminDashboardExcelExportServiceTests`（18件）・`AdminDashboardDialogLayoutTests`（15件）を新設、`MainWindowKeyBindingTests` に4件追加（+281）。SQL に閉じた挙動（日付境界・`COUNT(DISTINCT DATE(date))`・貸出中レコードの除外・月末レコードの選び方）はインメモリ SQLite の実 DB で固定する。式インジェクション対策の検証は表示文字列ではなく ClosedXML の `Style.IncludeQuotePrefix` で行う（サニタイズが付ける `'` は ClosedXML がテキストリテラル指示子として消費し表示値に現れないため、先頭文字を見ると「未サニタイズ」と誤検出する）。
  - 02_DB設計書 §5.2／03_画面設計書 §1・§2・§3.1・§3.23・§5.1／04_機能設計書 §20／05_クラス設計書 §3.2・§3.3・§6.1・§7.0・§12.3・付録A／06_シーケンス図 §16／07_テスト設計書 §1.1a（単体 4,110→4,391・合計 4,136→4,417 件）・§2 UT-073／管理者マニュアル §9.1・§9.4・付録A・クイックガイド／ユーザーマニュアル §3.1 を同期更新（#1692）
