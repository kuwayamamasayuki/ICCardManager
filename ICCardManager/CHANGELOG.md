...
- Issue #1763 **カード内に取り込む利用履歴が無い状態で交通系ICカードを新規登録したとき、台帳への初期残高の記録に失敗しても「登録しました」と表示される**問題を修正した（Issue #1727 の残件）。#1727 は「履歴が**ある**」経路について初期残高行と履歴行を同一トランザクションに束ね、リトライで包み、失敗をユーザーへ通知するようにしたが、**同じ `if` の反対側**は手つかずで、`_ledgerRepository.InsertAsync` を直接呼び例外を警告ログへ落とすだけだった。
  ...
  - 検証: `RegistrationLedgerFailureMessageTests`（13件）を新設、`CardManageViewModelTests` に6件・`LendingServiceHistoryImportTests` に3件追加（+22）。文言テストは**全ファクトリをリフレクションで列挙**して 3 要素品質を検証し、列挙が空振りしていないことも同じクラスで表明する。ViewModel の3件（成功扱いにしない／復旧案内／一覧再読込も失敗した場合の通知）は**修正前のコードに当てて実際に赤になること**を確認済み。
  - 05_クラス設計書 §5.1a／07_テスト設計書 §1.1a（単体 4,906→4,928・合計 4,932→4,954 件）・§2 UT-017j・UT-021・UT-086 を同期更新（#1763）
