...
- Issue #1739 **バックアップ健全性警告・残高不整合警告が、起動後の最初のカード操作で消えて二度と表示されなくなる**問題を修正した。
  ...
  - 04_機能設計書 §7.4（`WarningType` ごとの生成元・生成/除去の契機と「各チェックメソッドは自分が生成する種別だけを入れ替える」規約を新設）／07_テスト設計書 §1.1a（単体 4,524→4,532・合計 4,550→4,558 件）・§2.20c UT-WARN-KEEP を同期更新（#1739）
