...
- Issue #1781 **CSV インポートが一部エラーで終わるとプレビューが残り、押し直すと二重登録し得る**問題を修正した。
  ...
  - 03_画面設計書 §3.10（「インポート実行」ボタンの活性条件とプレビューの破棄）／07_テスト設計書 §1.1a（単体 4,591→4,598・合計 4,617→4,624 件）・§2 UT-028 No.23〜29／管理者マニュアル §5.6.5（「一部エラーで終わったときの対処」を追加）・§6.4（利用履歴詳細にスキップオプションが効かない旨の注意）を同期更新（#1781）
