...
- 管理者マニュアル §6.1「手動バックアップ」/ §6.2「リストア（復元）」にバックアップ完了ステータス・リストア用バックアップ一覧・ファイル選択ダイアログのスクリーンショット参照を追加。従来は §6.1 冒頭の `system.png`（システム管理画面全体）1 枚のみで、(a) 「バックアップを作成」ボタン押下後の完了通知の見え方、(b) リストア時に表示されるバックアップファイル一覧、(c) 「ファイルを指定してリストア」のファイル選択ダイアログという、リストアという**他のすべての PC でアプリ終了を要する不可逆操作**の事前確認画像が欠けていた。Issue 概要は「完了通知**ダイアログ**」を要求していたが、`SystemManageViewModel.CreateBackupAsync`（L132）は `SetStatus($"バックアップを作成しました: {ファイル名}", false)` でシステム管理画面下部のステータスバーに完了メッセージを表示する実装で、専用ダイアログは存在しない。`tools/TakeScreenshots.ps1` 行 488 のエントリにも「（完了通知ダイアログは現状未実装）」と明記されており、PR #1427 で `backup_completed_status.png` / `restore_list.png` / `restore_file_dialog.png` の 3 エントリが先行追加済み。本対応では実装変更（完了通知ダイアログの新規実装）ではなく**文書を実装に整合させる方針 A** を採用し、§6.1 step 3 の文言を「バックアップが設定済みの保存先に作成されます」→「バックアップが設定済みの保存先に作成され、ステータスバーに『バックアップを作成しました: <ファイル名>』と表示されます」に書き換えた上で `backup_completed_status.png` を `{width=50%}` で参照（既存 `system.png` と同幅で章内一貫性を維持）。リストア節は文言修正なしで step 2 直下に `restore_list.png` を `{width=50%}`、「ファイルを指定してリストア」段落直後に `restore_file_dialog.png` を `{width=60%}`（OS 標準ファイル選択ダイアログは横長で左ペイン+ファイルリスト+パス欄の可視性のため僅かに広く）で挿入。Issue ラベル `type: documentation` の意図に沿い、完了通知ダイアログの実装是非（UI 操作フロー全体に影響する設計判断）は別 Issue で議論する形でスコープを絞った。画像本体はユーザー側で別コミットで追加（Issue #1411 / #1412 / #1414 / #1415 / #1416 と同じ運用）。本コミットはマニュアル本文と CHANGELOG のみで、コード・テスト・撮影スクリプト変更なし（#1417）
