...
- Issue #1764 **完了メッセージ（「更新しました」「〇〇 を復元しました」等）が `CancelEdit()` に消されて一度も表示されない**欠陥に対し、**ソーステキスト上の静的検査**（`CompletionMessageOrderConventionTests`）を追加して再発を構造的に防ぐようにした。**Issue が指摘したカード管理画面の復元・更新の 2 経路は、起票後にマージされた Issue #1759（PR #1827）が「残る 10 経路」を一括是正したため既に修正済み**であり、本 PR に実装の変更は含まない（動作は変わらない）。
  ...
  - **コメントと文字列リテラルを剥がしてから検査する**。本番ソースには「`CancelEdit()` は `StatusMessage` をクリアするため〜」という由来コメントが各経路に置かれており、剥がさないと**規約を説明する文章そのもの**が違反として検出される（#1692 の極性の反転と同じ形）。補間文字列の `{ }` も剥がさないとブロック対応がずれ、検査が別の場所を見る。
  - 検証: `CompletionMessageOrderConventionTests`（7件）を新設（+7）。**修正前のコード（PR #1827 の親コミット `50e212b7`）に当てて各 5 件・合計 10 件の違反が検出されること**を実測で確認済み。検出された 10 件は `.claude/rules/error-messages.md` が #1759 で**手作業により列挙した**「残る 10 経路（カード: 更新・削除・払戻・復元×2／職員: 登録・更新・削除・復元×2）」と一致する。空振り検出は「実ファイルから違反が見つかること」ではなく**検査ロジック自体を既知のサンプル入力で固定**して行う（規約が守られている状態でテストが赤になると、修正者を「対象から外す」方向へ誘導するため。#1786 で確立した作法）。
  - 07_テスト設計書 §1.1a（単体 4,929→4,936・合計 4,955→4,962 件）・§2 UT-087 を同期更新（#1764）
