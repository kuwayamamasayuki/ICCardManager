...
- Issue #1808 **CSV インポートの無言欠陥 3 件**（親 Ledger 更新の握りつぶし・職員番号の幻の差分・エクスポート→取り込みの往復でのシングルクォート混入）を修正した。いずれも「静かに間違った状態を作る」性質のもの
  ...
  - 管理者マニュアル §8.5／07_テスト設計書 UT-016（#1808 表）・UT-SECURITY-003・§1.1a（単体 5,017→5,053・合計 5,043→5,079）／`.claude/rules/development-conventions.md`（`bool` 握りつぶしの兄弟ファイル・往復対称性・null と空文字の比較）を更新
