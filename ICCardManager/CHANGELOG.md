...
- Issue #1806 **履歴統合の取り消しが明細を rowid だけで移動し、「取り消し済み」マークとも原子的でない**問題を修正した。
  ...
  - **対処①**: 明細移動を `WHERE rowid = @rowid AND ledger_id = @targetId`（統合先に属していることを条件に含める。同ファイルの `UpdateDetailBusStopsAsync` と同じスコープ）にし、影響行数が 1 でなければ競合として `false` を返す（Issue #1753 の作法）。統合先の復元 UPDATE も 0 行なら `false`（統合後に統合先が削除された）。**統合後に明細が作り直された履歴は取り消せない**（rowid 以外に明細の同一性を確かめる手段が無い）ため、復元を中止して「統合後に編集・分割・削除された可能性」を案内する。表末尾の明細が同じ rowid・同じ順序で作り直された場合だけは検出できない（同じ台帳内の明細の入れ替わりに留まる。Undo データの形式変更を伴うため本 Issue では扱わない）
  ...
  - 検証: `LedgerRepositoryTransactionTests`（+8。rowid 再利用の交差破損・明細作り直し・統合先削除・tx ロールバック・マークの二重実行）、`LedgerMergeServiceUnmergeAtomicityTests`（新設 2。実 DB でマークだけを `BEFORE UPDATE … RAISE(ABORT)` で落とし、復元が巻き戻ること・再実行で統合元が 1 回だけ復活することを行数で表明）、`LedgerMergeServiceTests`（+4・既存 8 件を tx オーバーロードへ更新）
  - 04_機能設計書 §11.5／05_クラス設計書 §5.5b・§5.15／07_テスト設計書 §1.1a（単体 4,991→5,005・合計 5,017→5,031 件）・§2 UT-052a・UT-091／ユーザーマニュアル §5.4「統合を元に戻す」を同期更新（#1806）
