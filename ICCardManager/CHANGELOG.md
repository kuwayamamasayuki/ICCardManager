...
- Issue #1716 **共有モードのネットワーク切断が検知されず、メイン画面の切断トーストが出ない**問題を修正した。#1690 の実機検証で発見。共有フォルダのネットワークを切断しても `DbContext.CheckConnection()` が `true` を返し続け、利用者は警告を受け取れないまま操作を続けて**書き込み時に初めてエラーに気づく**状態だった（障害の早期通知という #1470 の目的が達成できていなかった）。
  ...
  - 検証: `DbContextCheckConnectionReachabilityTests`（15件）を新設（+15）。単体テストで SMB 切断は再現できないため、到達確認を `protected virtual` のテスト継ぎ目とし「クエリは通るがファイルへ届かない」状態を差し替えで作る。あわせて既定実装（`File.Exists` ベース）自体も実ファイルの有無で検証し、継ぎ目だけを検証して実装が空洞化することを防ぐ。既存の `DbContextCheckConnectionLoggingTests` / `DbContextCheckWritableTests` / `SharedModeMonitor*Tests` / `ConnectionDiagnosticsServiceTests` に影響がないことも確認済み。
  - 03_画面設計書 §3.22／04_機能設計書 §18.4・§19.2／05_クラス設計書 §3.3・§5.5／07_テスト設計書 §1.1a（単体 4,088→4,103・合計 4,114→4,129 件）・§2.55 UT-072 を同期更新（#1716）
