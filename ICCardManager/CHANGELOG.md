...
**バグ修正（インストーラー）**
- インストーラー実行中の「データベースの保存先」および「帳票出力先フォルダ」ページで、`net use` 等で設定したマップトドライブが「参照...」ボタンのフォルダ選択ダイアログに表示されない問題に対処した。原因はインストーラーが `PrivilegesRequired=admin`（管理者権限）で動作するため、UAC 昇格後のプロセスが標準ユーザーセッションのドライブマッピングを参照できないという Windows の仕様上の制約。対策として、レジストリ `HKCU\Network` からマップトドライブ情報（ドライブレター＋UNC パス）を列挙し、「マップトドライブから選択」コンボボックスをフォルダ選択 UI に追加した。マップトドライブが 1 つも検出されない場合はコンボボックスを非表示にし、従来と同一の画面構成を維持する。コンボからドライブを選択するとパス入力欄にドライブレターが自動入力され、DB 保存先ページでは「共有フォルダで複数 PC から使用」ラジオボタンも自動選択される。管理者マニュアル §2.2 にマップトドライブ操作の補足を追記（Issue #1584）
