...
- Issue #1726 **カード管理画面で備考などを編集して保存すると、紙の出納簿から引き継いだ累計と出納簿の開始ページ番号が消える**問題を修正した。
  ...
  - **操作ログに虚偽の変更を残さないため ViewModel 側でも値を引き継ぐ**。`OperationLogger` は `IcCard` 全体を JSON 化して `BeforeData` / `AfterData` に記録するため、DB を守るだけでは「開始ページ番号 7 → 1」「繰越受入 120,000 → 0」という**実際には起きていない変更**が監査ログに残る。引き継ぎ元は DB の最新値を優先し、取得できない場合（共有モードで他 PC が削除した直後など）は一覧の値で代替する。
  - **CSV インポートの更新経路は影響を受けていなかった**（Issue 本文は影響すると推定）。カード CSV の更新経路は DB から読み込んだエンティティのカード種別・管理番号・備考だけを差し替えて渡しており、4項目は元の値を保っていた。将来 CSV の列だけから `IcCard` を組む実装へ変わると同じ消失が起きるため、退行検出用のガードテストを追加した。
  - **既に値が失われたカードの復旧**: 本修正は今後の消失を防ぐもので、**過去に編集保存したカードの値は復元されない**。移行時の繰越累計・開始ページ番号が分かる場合は、`ic_card` テーブルの該当列を直接修正する必要がある（対象は紙出納簿から年度途中で移行したカードと、開始ページ番号を 1 以外で登録したカードのみ）。
  - 検証: `CardRepositoryTests` に2件（通常版・トランザクション版で登録時専用列が保全されること）・`CardManageViewModelTests` に2件（値の引き継ぎ、更新前データ取得不能時の代替）・`CsvImportServiceTests` に1件（CSV 経路のガード）を追加（+5）。修正を外すと前者4件が実際に失敗することを確認済み（CSV のガードテストは修正前からパスし、上記「影響を受けていなかった」の根拠になっている）。
  - 05_クラス設計書 §5.5b／07_テスト設計書 §1.1a（単体 4,420→4,425・合計 4,446→4,451 件）・§2 UT-016 No.34・UT-021 No.20/21・UT-025 No.28/29／`.claude/rules/development-conventions.md` を同期更新（#1726）
