...
- Issue #1741 **CSV インポートが全件成功したときだけ、監査ログに取り込み元ファイル名が記録されない**問題を修正した。
  ...
  - 検証: `DataExportImportViewModelTests` に3件追加（+3）。Moq の呼び出し記録ではなく、実 `OperationLogRepository` 経由で **DB に書かれた `operation_log` 行**（`target_id` / `after_data.FilePath` / `after_data.FileName` / `target_table`）を読み返して検証する。部分成功分岐は修正前から正しいが、非対称の再発防止としてピン留めした。
  - 05_クラス設計書 §5.17b／07_テスト設計書 §1.1a（単体 4,565→4,568・合計 4,591→4,594 件）・§2 UT-028 No.12〜14 を同期更新（#1741）
