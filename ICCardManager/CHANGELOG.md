...
- Issue #1739 **バックアップ健全性警告・残高不整合警告が、起動後の最初のカード操作で消えて二度と表示されなくなる**問題を修正した。
  ...
  - 検証: `MainViewModelTests` に7件、`MainViewModelIntegrationTests` に1件追加（+8）。**保持対象は `Enum.GetValues(typeof(WarningType))` から導出**しており、種別を新設すると自動的に検査対象に入る（保持リストの更新漏れという本 Issue の再発を機械的に防ぐのはこの1件）。単体テストは `CheckWarningsAsync` を直接呼ぶだけで故障シナリオを通らないため、職員証タッチ → 交通系ICカードタッチ → 返却という**実際の操作を経由して警告が残ること**を統合テストで別途表明した。修正前に全8件のうち6件が RED（残り2件は既存の残額警告挙動のピン留め）であることを確認済み。
  - 04_機能設計書 §7.4（`WarningType` ごとの生成元・生成/除去の契機と「各チェックメソッドは自分が生成する種別だけを入れ替える」規約を新設）／07_テスト設計書 §1.1a（単体 4,524→4,532・合計 4,550→4,558 件）・§2.20c UT-WARN-KEEP／管理者マニュアル §6.1「バックアップ状況（健全性の確認）」（通常操作では消えない旨を追記）／`.claude/rules/development-conventions.md`（「全消し＋再生成」で保持側を列挙しない規約を新設）を同期更新（#1739）
