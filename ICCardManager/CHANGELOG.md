...
- Issue #1731 **「最新残高」の取得が同一日内で実際の時系列を無視し、その日の最終残高ではない値を表示・記録し得る**問題を修正した。
  ...
  - **スコープ外**: ダッシュボードのグラフ用集計（`GetMonthEndBalancesByCardAsync` / `GetBalancesBeforeAsync`、#1692）は SQL 側で同じ id タイブレークを使うが、影響がグラフ表示に限られるため本修正には含めない（#1770 で別途追跡。テスト設計書 UT-020e に記録）。
