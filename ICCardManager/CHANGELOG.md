...
- Issue #1770 **管理者ダッシュボード（F8）の残高推移グラフが、その月の実際の月末残高ではなく中間残高で描かれ得る**問題を修正した（#1731 のスコープ外として残していた 2 本の是正）。
  ...
  - **修正**: SQL の役割を「（カード × 月）／（カード）ごとの**最終稼働日**を絞る」までに留め、その日の**全行**を C# へ返して `ResolveChainFinalBalanceAsync` がチェーンを解決する方式に改めた。台帳は 6 年分保持されるため、**全件を C# へ読み出さず最終稼働日の行だけに絞る**作法（#1692 の設計判断）は維持している（C# へ渡るのは(カード × 月)あたり数行）。既存インデックスで足りるため**マイグレーション不要**、`ILedgerQueryService` のシグネチャも不変。
