...
- Issue #1770 **管理者ダッシュボード（F8）の残高推移グラフが、その月の実際の月末残高ではなく中間残高で描かれ得る**問題を修正した（#1731 のスコープ外として残していた 2 本の是正）。
  ...
  - 02_DB設計書 §5.2／04_機能設計書 §20.2・§20.4／07_テスト設計書 §1.1a（単体 4,939→4,944・合計 4,965→4,970 件）・§2 UT-020e・UT-020f・UT-073 件数表／`.claude/rules/business-logic.md`「同一日内の順序は id では決まらない」を同期更新（#1770）
