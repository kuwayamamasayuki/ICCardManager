...
- Issue #1716 **共有モードのネットワーク切断が検知されず、メイン画面の切断トーストが出ない**問題を修正した。#1690 の実機検証で発見。共有フォルダのネットワークを切断しても `DbContext.CheckConnection()` が `true` を返し続け、利用者は警告を受け取れないまま操作を続けて**書き込み時に初めてエラーに気づく**状態だった（障害の早期通知という #1470 の目的が達成できていなかった）。
  ...
  - **到達確認には上限時間（5秒）を設ける**（実機フィードバックを受けた追加修正）。到達不能な UNC への `File.Exists` は下位の TCP/SMB タイムアウトまで戻らず、**実測で 21〜42 秒ブロックする**（直後の再呼び出しは Windows のネガティブキャッシュにより 0 秒）。上限のない初版では「切断トーストが出るまで約1分かかる」（15秒待って確認開始 → 最大42秒ブロック → 完了して初めて状態遷移。その間の Tick は実行中フラグで全てスキップ）、および「**トーストが出たり出なかったりする**」（ブロック中にネットワークが復旧すると進行中の確認がそのまま成功し、状態が `Connected` のままで遷移が起きない＝切断を一度も検知しない）という不具合が実機で観測された。別スレッドで実行し `AppConstants.DatabaseReachabilityProbeTimeoutSeconds`（5秒）以内に完了しなければ到達不能とみなすことで、**切断検知は最悪でも 15 + 5 = 20 秒**に収まる。5秒は正常な LAN での `File.Exists`（10ミリ秒未満）の約500倍の余裕で、遅いだけの共有フォルダを切断と誤判定しにくい水準。
  - **進行中の到達確認は 1 本に限定**した。上限で打ち切っても `File.Exists` 自体は中断できない（キャンセル手段がない）ため、打ち切るたびに新しい確認を始めるとブロック済みスレッドが15秒ごとに積み上がる。
