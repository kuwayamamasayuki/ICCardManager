...
- Issue #1805 **返却のコミット後処理で例外が出ると「返却失敗」と誤表示され、再タッチで貸出として再記録される**問題を修正した。
  ...
  - **修正**: `Success` を「台帳への記録が確定した」ことだけを表す値とし、30秒ルール用の処理情報と併せて**コミット直後（`PersistReturnAsync` の戻り直後）に確定**させた。以降の後処理で出た例外は `catch (Exception) when (result.Success)` で吸収し、`Success` / `ErrorMessage` は変えずに新設の `LendingResult.HasPostCommitFailure` を true にする（`LogWarning`。返却自体は成功しているため Error ではない）。`MainViewModel.HandleReturnSuccessAsync` はこのとき `Balance` / `IsLowBalance` / `WarningBalance` を信頼できないため残額付きの返却通知を出さず、「返却は記録済み」＋「残額を確認できませんでした。再タッチしないでください。」＋中立的な `SoundType.Warning` で案内する（#1725 と同じ判断。「もう一度タッチ」と案内すると30秒ルールの逆処理で記録済みの返却が取り消される）。以降の後処理（一覧・ダッシュボード・履歴の更新、バス停入力ダイアログ）は通常どおり続行する。`.claude/rules/development-conventions.md`「コミット確定後の後処理を、成否の判定に巻き込まない」（#1727）の返却経路への適用で、**規約が書かれる前から存在した経路が取り残されていた**形。`LendAsync` はコミット直後に処理情報を確定し後処理の DB I/O を持たないため対象外
  - **記録前の失敗は従来どおり**「失敗・再タッチ」で案内し、処理情報も残さない（`Success` を前倒ししたことで本当に失敗したケースまで成功扱いにすると返却漏れになるため、両方向をテストで固定）
  - 検証: `LendingServiceTests` に5件・`MainViewModelIntegrationTests` に3件を追加（+8）。`HasPostCommitFailure` プロパティだけを先に足した状態で 5 件が想定どおりの理由（`Success` が false／`LastProcessedCardIdm` が null／`ShowError("エラー", "返却処理でエラーが発生しました: database is locked")` が呼ばれた）で赤になることを確認済み。統合テストの失敗注入は `GetAppSettingsAsync` を**最初の 1 回だけ** throw させ、返却後のダッシュボード更新が同じ原因で失敗して #1725 の経路と重なるのを避けた
  - 03_画面設計書 §3.2／04_機能設計書 §2.5.1（新設）／05_クラス設計書 §3（`LendingResult` に `WarningBalance` / `MayHaveIncompleteHistory` / `HasPostCommitFailure` を追記）・§5.5 注記／06_シーケンス図 §4／07_テスト設計書 §1.1a（単体 4,982→4,990・合計 5,008→5,016 件）・UT-090（新設）／ユーザーマニュアル §9.3（「◯◯は記録済み」トーストの対処を追記）／`.claude/rules/development-conventions.md`・`business-logic.md` を同期更新（#1805）
