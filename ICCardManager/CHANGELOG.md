...
- Issue #1760 **共有モードの競合で、カードや職員の変更が確定したのに操作ログに 1 行も残らない**場合がある問題を修正した。カード編集・払い戻し・職員編集は操作ログ用に更新前データを `GetByIdmAsync`（`is_deleted = 0`）で読んでから書き込むが、読み取りが null のとき `if (before != null)` で**ログ記録だけを飛ばして書き込みは続行**していた。通常は書き込み側の WHERE も 0 行になるため到達しないが、**読み取りと書き込みの間に他の PC が対象を復元すると書き込みだけが成功**し、カード種別・管理番号・備考が実際に書き換わったまま `operation_log` には何も残らない。`operation_log` は 6 年保存される唯一の監査記録であり、「誰がいつ何を変更したのか分からない変更」が残ることは、誤った記録が残るのと同等以上に問題になる。
  ...
  - **同型の欠陥が残っていた 2 か所も併せて是正した**（`.claude/rules/development-conventions.md`「規約を新設したら同種の既存箇所を横断で洗う」）。カードの払い戻し（`RefundAsync`）と職員編集（`StaffManageViewModel.SaveAsync`）が同じ形で、Issue 本文の対象（カード編集）だけを直すと残り 2 経路が確実に次の Issue になる
  ...
  - 検証: `CardManageViewModelTests` +5件・`StaffManageViewModelTests` +3件・`ConcurrencyConflictMessageTests` +2件（+10）。「ログが残ったか」は `OperationLogger` のログ記録メソッドが `virtual` ではなくモックで検証できないため、本物の実装が書き込む `IOperationLogRepository` で表明する。修正前に 4 件が実際に赤になることを確認済み（残る 2 件は成功パスの回帰固定）。あわせて既存の更新成功テスト 5 件へ「対象行が存在する」モック設定を追加した — 従来は loose mock の既定値（null）により「読み取りは null なのに UPDATE は 1 行に一致する」という**実 DB では競合時にしか起きない状態**を前提にしていた（`.claude/rules/testing.md`「モック構成が実 DB で成立し得るかを確かめる」、Issue #1728）。アサーションは変更していない
  - 05_クラス設計書 §10.4・§10.4a／07_テスト設計書 §1.1a（単体 4,878→4,886・合計 4,904→4,912 件）・§2 UT-084 を同期更新（#1760）
