...
- 同構造の Fact / Theory テスト群 5 クラス計 27 メソッドを `[Theory] + [InlineData]` に統合し、テストコードを DRY 化（テストケース数は不変、単体テスト 3,227 件・UI テスト 26 件・合計 3,253 件を維持）。対象: `FileSizeConverterTests`（Fact 2 + Theory 2 → 1 Theory、InlineData 7 件）、`PathValidatorTests` Traversal 系（Fact 6 + Theory 2 → 1 Theory、InlineData 11 件、`SafeLookalikePaths` Theory は逆方向検証のため分離維持し旧 `#region "Issue #1268: 強化されたパストラバーサル検出"` を「トラバーサル誤検出防止」に改名）、`FormulaInjectionSanitizerTests` False 系（Theory 2 → 1 Theory、InlineData 9 件、True 系は危険判定の仕様ドキュメント性維持のため分離）、`SharedModeMonitorTests` `UpdateSyncDisplayText` テキスト系（Theory 3 → 1 Theory、InlineData 10 件、Stale 判定 Theory・初期状態 Fact は Assert 対象が異なるため分離）、`StationMasterServiceTests`（Theory 10 → 1 Theory、InlineData 65 件、路線別コメント区切りでレビュー性確保、TOICA の Area 2 優先業務ロジック説明コメントも保存）。テストメソッド名は xUnit `[Theory]` 化に合わせて「意図を表す総称名」に刷新（例: `Convert_バイト単位の場合B表示になること` 他 3 メソッド → `Convert_バイト数を適切な単位で表示すること`）。本体コード変更なし、`dotnet test` は全件 Pass（リファクタリング前後の `dotnet test --list-tests` を diff したところメソッド名置換のみで件数完全一致）。`docs/superpowers/specs/2026-05-18-test-suite-cleanup-design.md`「Theory 化候補（別 Issue で対応）」で予告されていた追加リファクタリングの完遂。設計書 `ICCardManager/docs/superpowers/specs/2026-05-18-issue-1550-theory-consolidation-design.md` に方針（True/False 分離維持・命名総称化）を記録（#1550）
