...
- 新規職員登録ダイアログで職員証タッチ後に氏名欄へ自動フォーカス (Issue #1429)（#1429）
- PaSoRi 未接続時のステータスバー誤表示を修正 (Issue #1428)（#1428）

**ドキュメント**
- マニュアル参照済みスクリーンショット6点の取りこぼし補完（#1448）
- 管理者マニュアル §2.2 に「帳票出力先」インストーラーページの説明を追加 (Issue #1247)（#1247）
- マニュアルバージョン記載とテスト統計の実装同期（#1446）
- 管理者マニュアル §2.6 アンインストール時のデータ取り扱い選択ダイアログのスクリーンショット参照追加 (Issue #1413)（#1413）
...
- 管理者マニュアル §5.6.5/§6.4 CSVインポートプレビュー画面のスクリーンショット参照追加 (Issue #1416)（#1416）
- 管理者マニュアル §5.3/§5.5 交通系ICカード編集・払い戻しダイアログのスクリーンショット追加 (Issue #1415)（#1415）
- 管理者マニュアル §4.5 再登録手順を実装に整合 (Issue #1437)（#1437）
- 管理者マニュアル §4 職員登録・編集ダイアログのスクリーンショット追加 (Issue #1414)（#1414）
- ユーザーマニュアル §3.3 カード一覧の状態別スクリーンショット追加 (Issue #1412)（#1412）
- ユーザーマニュアル §9.3/§9.5 エラー表示のスクリーンショット参照を追加 (Issue #1411)（#1411）
- ユーザーマニュアル §7.2 各設定項目のスクリーンショット追加 (Issue #1409)（#1409）
- 管理者マニュアル付録 B 用語集を 12 項目へ拡充 (Issue #1420)（#1420）
- ユーザーマニュアル巻末に「付録 A 用語集」を追加 (Issue #1419)（#1419）
- 自動バックアップの保持世代数（30）をマニュアルに明記 (Issue #1408)（#1408）
