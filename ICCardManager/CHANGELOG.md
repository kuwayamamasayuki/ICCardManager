...
- Issue #1727 **交通系ICカードの登録時に利用履歴の取込が失敗しても無視され「登録しました」と表示される**問題を修正した。
  ...
  - **成功メッセージが表示されていなかった問題も併せて解消**: `CancelEdit()` が `StatusMessage` / `IsStatusError` をクリアするのに、登録処理は `StatusMessage = "登録しました"` を `CancelEdit()` の**前**に設定していたため、実際には何も表示されていなかった。結果表示を後処理のあとへ移した（**カードの復元・更新には同じ問題が残っている** → Issue #1764 で追跡）。
  ...
  - 検証: `LendingServiceHistoryImportTests` を新設（12件。SQLITE_BUSY のリトライ吸収、失敗時の `Success=false`／`ImportedCount=0`／理由文言、生の例外文言を漏らさないこと、**共有／ローカル両モードでの原因の書き分け**、**コミット後の後処理で例外が出ても成功として返すこと**、**実 `LedgerRepository` を噛ませて履歴書込み失敗時に初期残高行がロールバックされること**、成功時に初期残高行＋履歴が同時確定すること）、`CardManageViewModelTests` に7件追加（失敗時に成功表示しない／復旧案内ダイアログ／一覧更新と編集終了／メッセージ品質／成功時の回帰／**一覧再読込も失敗したときの通知**／成功パスで握りつぶさない境界）（+19）。ロールバックの検証はモックでは観測できないため、実リポジトリで**実際の `ledger` 行数**を表明している。
  - 05_クラス設計書 §5.1・§5.1a／07_テスト設計書 §1.1a（単体 4,425→4,444・合計 4,451→4,470 件）・§2 UT-017j・UT-025 No.29〜35／管理者マニュアル §5.2・§10.3 を同期更新（#1727）
