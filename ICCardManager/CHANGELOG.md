...
- Issue #1715 **03_画面設計書 §1 画面遷移図**の実装との乖離3件を修正し、再発防止の規約テストを新設した。§2 画面一覧と `Views/Dialogs/*.xaml` は一致していたのに遷移図だけが取り残されており、設計書の冒頭にある「システムの全体像」が古いまま後続の節より先に読まれる状態だった。
  ...
  - 再発防止として `ScreenTransitionDiagramConsistencyTests`（11件）を新設。§1 遷移図 ↔ §2 画面一覧 ↔ 実在 XAML ↔ 実装（`RequestAuthenticationAsync` の呼び出し元）を**双方向で**突合する。片方向では「図にあるが実装には無い」誤記（上記の `LEDGERROWEDIT` 誤記がまさにこれ）を見逃すため。領域ノードは許容リスト `AreaNodes` で除外しつつ、リスト自体もラベル・§3 見出しとの一致・「〜ダイアログ」を名乗らないことを検証し、除外が名称の誤りを素通しさせないようにした。認証操作名は集合をスナップショット固定し、既存ノードに操作が増えた場合（エッジ本数では検出できない）も設計書の更新を強制する。修正前の設計書に対して4件が失敗することを確認済み。
  - 03_画面設計書 §1・§3.17／07_テスト設計書 §1.1a（単体 4,076→4,087・合計 4,102→4,113 件）・§2.54 UT-SCREEN-DIAGRAM-001 を同期更新（#1715）
