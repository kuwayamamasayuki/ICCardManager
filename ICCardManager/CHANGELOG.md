...
- Issue #1807 **モーダルダイアログ表示中もカード読み取りが有効で、ダイアログが多重に開いたり背後で貸出・返却が進んだりする**問題を修正した。原因は共通で、**カード読み取り抑制（Issue #852 の `CardReadingSuppressedMessage`）の取得と解放が処理範囲と一致していない**こと。
  ...
  - **対処①②**: `HandleUnregisteredCardAsync` の全区間（事前読み取り → 種別選択 → 登録ダイアログ → 一覧更新）を新設の抑制ソース `CardReadingSource.UnregisteredCardDialog` で囲む。`MainViewModel.BeginCardReadingSuppression` が返す `IDisposable` を `using` で保持し、解放を Dispose（finally 相当）で保証する（Issue #1725 の「解除は finally で保証する」と同じ判断）。ダイアログ側の抑制ソースとは独立しており、片方の解放でもう片方は解けない
  ...
  - テストは `MainViewModelTests`（`ShowDialog` のモックの Callback 内でカード読み取りを発火させ、多重表示・背後処理・二重購読の不在と閉じた後の解放を表明）4 件、`StaffManageViewModelTests` 5 件（`WeakReferenceMessenger` に記録用の受信者を登録し送信列を表明。`OnCardRead` の本体を `HandleCardReadAsync` へ分離してテストから到達可能にした）、`CardManageViewModelTests` 1 件を追加。修正前のコードで新規 8 件が赤になることを確認した。03_画面設計書 §3.3 / §3.9 に抑制の取得・解放の対応表を追加
