...
- Issue #1740 **履歴行の修正画面で「自動計算」を ON にすると、残高が前の行を無視して再計算され、正しい残高が破壊される**問題を修正した。
  ...
  - 検証: `LedgerRowEditViewModelTests` に8件・`MainViewModelTests` に4件・`LedgerRowEditDialogAutoBalanceLayoutTests`（新設）に4件（+16）。回帰テストが実際に本不具合を捕まえることは、`PreviousBalance` の設定を修正前の状態へ戻す変異注入で確認した（該当2件が失敗する）。ViewModel のテストは XAML の結線漏れを検出できないため、チェックボックスの無効化は XAML テキスト上の静的検証で別途固定した（`CardManageDialogStatusAreaLayoutTests` と同方針）。
  - 03_画面設計書 §3.12／07_テスト設計書 §1.1a（単体 4,536→4,552・合計 4,562→4,578 件）・§2.22a UT-029a4〜UT-029a6／ユーザーマニュアル §5.5 を同期更新（#1740）
