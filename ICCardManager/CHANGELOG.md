...
- Issue #1688 **帳票出力前プリフライトチェック**を追加した（2026-07-10 ユーザビリティ総合検討 Phase 2）。月次の物品出納簿作成は管理者の最頻定例作業だが、従来は出力してから Excel を開いて月計・繰越・負残高を目視確認する運用（管理者マニュアル §5.6.6）で、問題に気づくと「修正 → 再出力」の手戻りや検収での差戻しが発生していた。出力前に機械的に検出し警告として提示する。
  ...
  - テスト: `ReportPreflightCheckerTests`（検出ロジック21件＋`CheckAsync` 経路4件の計25件）・`ReportPreflightViewModelTests`（7件）を新設、`ReportViewModelTests` に8件追加。判定ロジックは `internal static` の純粋関数に切り出し、DB を介さず境界値を検証する。続行パスは実ファイル生成（テンプレート未配置時に `MessageBox.Show` でテストがブロックする）を避けるため `RunPreflightBeforeCreateAsync` を `internal` で公開して検証。
  - 03_画面設計書 §3.6・§3.21／04_機能設計書 §10a・§13.5／07_テスト設計書 §1.1a（単体 3,792→3,832・合計 3,818→3,858 件）・UT-REPORT-PREFLIGHT-001〜003／管理者マニュアル §5.6.6 を同期更新（#1688）
