...
- Issue #1794 **ダイアログの背後にあるボタンが押せてしまう**問題を修正した（Issue #1793 の続き）。
  ...
  - **オーナーの選択規則を `Common/DialogOwnerResolver` に持たせた**。優先順位は **アクティブなウィンドウ → モーダル子ウィンドウに塞がれていない（`IsEnabled`）ウィンドウのうち最後 → 使用可能な最後のウィンドウ**。**`Application.MainWindow` を特別扱いしない** — モーダルダイアログが開いている間にメイン画面をオーナーにすると、そのダイアログが無効化されず欠陥がそのまま残るため。候補は「表示済みでウィンドウハンドルを持ち、**アクティブ化する**（`ShowActivated`）」ものに限る（未表示・クローズ済みのウィンドウを渡すと `InvalidOperationException` になる）。解決できないときは従来どおりオーナー無しで表示する（表示しないより望ましい）。
  ...
  - 検証: `DialogOwnerResolverTests`（新設13）・`DialogServiceOwnerTests`（新設6。実行ケース数は Theory 展開で10）。**「ownerless になったか」は xUnit から観測できない**ため、オーナーを**選ぶ規則**を WPF 非依存の純関数へ切り出して網羅し、`Window` 実体を要するのは「継ぎ目を通っているか」の 1 件に絞った（`Window` は STA スレッドでしか生成できない）。静的検査は迂回コードを実際に入れて赤になることを確認済み。実際にクリックが遮られることの確認は Windows 実機での手動検証による。
  - 03_画面設計書 §3.10／05_クラス設計書 §5.18・§5.18a／07_テスト設計書 §1.1a（単体 4,960→4,979・合計 4,986→5,005 件）・§2 UT-089 を同期更新（#1794）
