...
sequenceDiagram
    ...
    BS->>BS: 一時ファイルへ SQLite Backup API でコピー<br/>（backup_….db.{GUID}.tmp）
    Note over BS: Issue #1748: 最終名へ直接書くと、コピー途中で失敗した<br/>「マジックヘッダだけ揃った切り詰めファイル」が<br/>有効な世代として残る。GUID は共有モードで<br/>複数台が同一の一時ファイルを開くのを防ぐ
