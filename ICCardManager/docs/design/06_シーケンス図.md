# シーケンス図

## 0. このドキュメントについて

| 項目 | 内容 |
|------|------|
| **対象読者** | 本システムの開発者・保守担当者。業務フローをオブジェクト間の時系列で追いたい人 |
| **前提知識** | `04_機能設計書.md`(業務仕様) / `05_クラス設計書.md`(登場するクラスの責務) |
| **このドキュメントで答えられる問い** | ・貸出/返却処理の中でクラス間でどうメッセージが飛ぶか<br>・30秒ルールやタイムアウトはどう実装されているか<br>・履歴統合/分割/職員認証のフロー<br>・バックアップ・CSV入出力・印刷プレビューの処理順 |
| **関連ドキュメント** | `04_機能設計書.md`(業務視点) / `05_クラス設計書.md`(実装視点) |

> **Mermaidの読み方**: `participant X as Y` は「X というエイリアスで Y を登場させる」、`A->>B: msg` は同期呼び出し、`B-->>A: resp` は戻り値。`alt` / `loop` / `Note` ブロックで条件分岐・繰り返し・補足を表現します。

> **トランザクション表記について（Issue #1245）**: 本書のシーケンス図では書き込み系処理を `await BeginTransactionAsync()` → `TransactionScope (using)` → `scope.Commit()` の形で表記しています。`scope.Rollback()` は原則として図に出しません — 例外経路では **`scope.Dispose()` が自動的にロールバックする** ためです。この仕様の詳細は [§9.1 TransactionScope の自動ロールバック仕様](#91-transactionscope-の自動ロールバック仕様重要) を参照してください。

---

## 1. シーケンス図マップ

本書には14本のシーケンス図が含まれます。機能グループで整理しました。

| グループ | 章 | 図 | 関連する業務仕様 |
|---|---|---|---|
| **中核フロー** | §2 | 職員証〜交通系ICカードタッチの全体 | 04 §2 |
| | §3 | 貸出処理 | 04 §2.2 |
| | §4 | 返却処理 | 04 §2.4 |
| | §6 | 30秒ルール適用 | 04 §3 |
| | §7 | 未登録カード処理 | - |
| | §8 | タイムアウト処理 | 04 §1.3 |
| | §9 | エラー処理 | - |
| **帳票・出力** | §5 | 月次帳票出力 | 04 §8 |
| | §15 | 印刷プレビュー | 04 §9 |
| **履歴管理** | §10 | 履歴統合(マージ) | 04 §11 |
| | §11 | 履歴分割 | 04 §12 |
| **セキュリティ** | §12 | 職員認証 | 04 §14 |
| **運用** | §13 | バックアップ/リストア | - |
| | §14 | CSV入出力 | - |

> **読み方のコツ**: まず §2〜§4 の3本(職員証→交通系ICカード→貸出/返却)を順に見れば、システムの心臓部が掴めます。他の図は必要になった時だけ参照してください。

---

## 2. 職員証タッチ〜交通系ICカードタッチのフロー

```mermaid
sequenceDiagram
    autonumber
    participant User as ユーザー
    participant PaSoRi as PaSoRi
    participant Reader as ICardReader
    participant VM as MainViewModel
    participant Sound as ISoundPlayer

    Note over User,Sound: 初期状態: WaitingForStaffCard

    User->>PaSoRi: 職員証をタッチ
    PaSoRi->>Reader: カード検出
    Reader->>VM: CardRead イベント発火(IDm)

    VM->>VM: 職員証かどうか判定
    alt 職員証の場合
        VM->>VM: 状態を WaitingForIcCard に変更
        VM->>VM: タイムアウトタイマー開始(60秒)
        VM->>VM: 画面表示更新
    else 交通系ICカードの場合（職員証なしで直接タッチ）
        VM->>VM: 履歴表示ダイアログを開く
    end

    Note over User,Sound: 状態: WaitingForIcCard

    User->>PaSoRi: 交通系ICカードをタッチ
    PaSoRi->>Reader: カード検出
    Reader->>VM: CardRead イベント発火(IDm)

    VM->>VM: 30秒ルールチェック
    alt 30秒以内の再タッチ
        VM->>VM: 逆処理を実行
    else 通常処理
        VM->>VM: カードの貸出状態を確認
        alt 未貸出（is_lent = 0）
            VM->>VM: 貸出処理を実行
            VM->>Sound: Play(SoundType.Lend)
        else 貸出中（is_lent = 1）
            VM->>VM: 返却処理を実行
            VM->>Sound: Play(SoundType.Return)
        end
    end

    VM->>VM: 状態を WaitingForStaffCard に戻す
    VM->>VM: 画面表示更新
```

---

## 3. 貸出処理シーケンス

```mermaid
sequenceDiagram
    autonumber
    participant VM as MainViewModel
    participant LS as LendingService
    participant SR as IStaffRepository
    participant CR as ICardRepository
    participant LR as ILedgerRepository
    participant DB as DbContext
    participant Sound as ISoundPlayer

    VM->>LS: LendAsync(staffIdm, cardIdm)

    LS->>CR: GetByIdmAsync(cardIdm)
    CR->>DB: SELECT FROM ic_card
    DB-->>CR: カード情報
    CR-->>LS: IcCard

    alt カード未登録
        LS-->>VM: Error: カードが登録されていません
    else カード貸出中
        LS-->>VM: Error: このカードは既に貸出中です
    end

    LS->>SR: GetByIdmAsync(staffIdm)
    SR->>DB: SELECT FROM staff
    DB-->>SR: 職員情報
    SR-->>LS: Staff

    alt 職員未登録
        LS-->>VM: Error: 職員証が登録されていません
    end

    LS->>DB: await BeginTransactionAsync()
    DB-->>LS: TransactionScope (using)
    Note over LS,DB: 以降の DB 操作は TransactionScope 内で実行

    LS->>LR: InsertAsync(Ledger)
    Note right of LR: summary = "（貸出中）"<br/>is_lent_record = true
    LR->>DB: INSERT INTO ledger
    DB-->>LR: ledgerId
    LR-->>LS: ledgerId

    LS->>CR: UpdateLentStatusAsync(cardIdm, true, now, staffIdm)
    CR->>DB: UPDATE ic_card SET is_lent = 1
    DB-->>CR: success
    CR-->>LS: success

    LS->>DB: scope.Commit()
    Note over LS,DB: using 脱出時に scope.Dispose() が<br/>トランザクションとリースを順次解放

    LS->>LS: LastProcessedCardIdm = cardIdm
    LS->>LS: LastProcessedTime = now
    LS->>LS: LastOperationType = Lend

    LS-->>VM: LendingResult(Success = true)

    VM->>Sound: Play(SoundType.Lend)
    VM->>VM: 画面表示更新（貸出完了）
```

---

## 4. 返却処理シーケンス

```mermaid
sequenceDiagram
    autonumber
    participant VM as MainViewModel
    participant Reader as ICardReader
    participant LS as LendingService
    participant SR as IStaffRepository
    participant CR as ICardRepository
    participant LR as ILedgerRepository
    participant SG as SummaryGenerator
    participant DB as DbContext
    participant Sound as ISoundPlayer

    VM->>Reader: ReadHistoryAsync(cardIdm)
    Reader-->>VM: List<LedgerDetail>

    VM->>LS: ReturnAsync(staffIdm, cardIdm, usageDetails)

    LS->>CR: GetByIdmAsync(cardIdm)
    CR-->>LS: IcCard(is_lent = true)

    LS->>SR: GetByIdmAsync(staffIdm)
    SR-->>LS: Staff

    LS->>LR: GetLentRecordAsync(cardIdm)
    LR->>DB: SELECT FROM ledger WHERE is_lent_record = 1
    DB-->>LR: 貸出レコード
    LR-->>LS: Ledger

    LS->>LS: 貸出時刻以降の利用履歴を抽出

    LS->>DB: await BeginTransactionAsync()
    DB-->>LS: TransactionScope (using)

    loop 日付ごとにグループ化
        alt チャージあり
            LS->>LR: InsertAsync(ChargeLedger)
            Note right of LR: summary = "役務費によりチャージ"
        end

        alt 利用あり
            LS->>SG: Generate(usageDetails)
            SG-->>LS: "鉄道（A駅～B駅）"
            LS->>LR: InsertAsync(UsageLedger)
            LS->>LR: InsertDetailsAsync(ledgerId, details)
        end
    end

    LS->>LR: UpdateAsync(lentRecord)
    Note right of LR: is_lent_record = false<br/>returner_idm = staffIdm<br/>returned_at = now

    LS->>CR: UpdateLentStatusAsync(cardIdm, false, null, null)

    LS->>DB: scope.Commit()
    Note over LS,DB: using 脱出時に scope.Dispose() が<br/>トランザクションとリースを順次解放

    LS->>LS: バス利用チェック
    alt バス利用あり
        LS->>LS: HasBusUsage = true
    end

    LS->>LS: 30秒ルール用の処理情報を記録・Success = true
    Note right of LS: Issue #1805: ここで返却は確定。<br/>以降の後処理の失敗で Success は落とさない

    LS->>LS: 残高チェック（残額の解決・残額警告の付与）
    alt 残高 < 警告閾値
        LS->>LS: IsLowBalance = true
    else 残額の解決・残額警告の DB I/O で例外
        LS->>LS: HasPostCommitFailure = true（Success は true のまま）
    end

    LS-->>VM: LendingResult(Success = true)

    alt HasPostCommitFailure
        VM->>Sound: Play(SoundType.Warning)
        VM->>VM: 「返却は記録済み／残額を確認できませんでした。再タッチしないでください。」
    else
        VM->>Sound: Play(SoundType.Return)
        VM->>VM: 残額付きの返却通知
    end

    alt バス利用あり
        VM->>VM: バス停入力ダイアログを表示
    end

    alt 残高警告
        VM->>Sound: Play(SoundType.Warning)
        VM->>VM: 警告メッセージを表示
    end

    VM->>VM: 画面表示更新（返却完了）
```

---

## 5. 帳票出力シーケンス

```mermaid
sequenceDiagram
    autonumber
    participant User as ユーザー
    participant VM as ReportViewModel
    participant RS as ReportService
    participant CR as ICardRepository
    participant LR as ILedgerRepository
    participant XL as ClosedXML

    User->>VM: 作成ボタンクリック

    VM->>VM: 入力値検証

    loop 選択されたカードごと
        VM->>RS: CreateMonthlyReportAsync(cardIdm, year, month, outputPath)

        RS->>CR: GetByIdmAsync(cardIdm)
        CR-->>RS: IcCard

        RS->>LR: GetByMonthAsync(cardIdm, year, month)
        LR-->>RS: List<Ledger>

        RS->>RS: 貸出中レコードを除外

        RS->>XL: new XLWorkbook(TemplatePath)
        XL-->>RS: Workbook

        RS->>RS: SetHeaderInfo(worksheet, card, year, month)

        alt 4月の場合
            RS->>LR: GetCarryoverBalanceAsync(cardIdm, year - 1)
            LR-->>RS: 前年度残高（繰越行として行セットに合成）
        end

        loop 行セット（繰越行・データ行）ごと
            RS->>RS: WriteReportRow(worksheet, row, reportRow)
        end
        Note right of RS: Issue #1023: WriteCarryoverRow / WriteDataRow を<br/>WriteReportRow に統合。RowType（繰越/データ）で出し分け

        RS->>RS: WriteTotalRow(月計)

        alt 3月の場合
            RS->>LR: GetByDateRangeAsync(cardIdm, fiscalYearStart, fiscalYearEnd)
            LR-->>RS: 年度の全レコード
            RS->>RS: WriteTotalRow(累計)
            RS->>RS: WriteCarryoverToNextYearRow()
        end

        RS->>XL: SaveAs(一時ファイルのストリーム)
        Note right of RS: Issue #2040: 年度ファイルへ直接保存しない。<br/>{年度ファイル名}.{8桁}.tmp へ書き切ってから<br/>File.Replace（新規は File.Move）で差し替える
        RS->>RS: CommitTempFile(tempPath, outputPath)

        RS-->>VM: true
    end

    VM->>VM: 完了メッセージ表示
    VM-->>User: 作成完了
```

---

## 6. 30秒ルール適用シーケンス

```mermaid
sequenceDiagram
    autonumber
    participant User as ユーザー
    participant VM as MainViewModel
    participant LS as LendingService
    participant Sound as ISoundPlayer

    Note over User,Sound: 前提: 直前に貸出処理を実行済み<br/>LastProcessedCardIdm = "CARD001"<br/>LastOperationType = Lend

    User->>VM: 同じカードを再タッチ（30秒以内）

    VM->>LS: IsRetouchWithinTimeout("CARD001")
    LS->>LS: LastProcessedCardIdm == cardIdm?
    LS->>LS: (Now - LastProcessedTime) <= 30秒?
    LS-->>VM: true

    VM->>VM: 操作者を決定（Issue #1729）

    alt _currentStaffIdm が確定済み（直前に職員証タッチ）
        Note over VM: いまタッチした職員をそのまま使う<br/>前回操作者で上書きしない
    else 未確定（職員証タッチ待ちからの再タッチ）
        VM->>VM: _currentStaffIdm = _lastProcessedStaffIdm
        Note over VM: 前回操作者も未設定ならエラー通知して中断
    end

    VM->>VM: LastOperationType を確認

    alt LastOperationType == Lend
        Note over VM: 前回が貸出なので、返却処理を実行
        VM->>LS: ReturnAsync(staffIdm, cardIdm, [])
        LS-->>VM: LendingResult(Success, OperationType = Return)
        VM->>Sound: Play(SoundType.Return)
    else LastOperationType == Return
        Note over VM: 前回が返却なので、貸出処理を実行
        VM->>LS: LendAsync(staffIdm, cardIdm)
        LS-->>VM: LendingResult(Success, OperationType = Lend)
        VM->>Sound: Play(SoundType.Lend)
    end

    VM->>VM: 画面表示更新
```

---

## 7. 未登録カード処理シーケンス

```mermaid
sequenceDiagram
    autonumber
    participant User as ユーザー
    participant VM as MainViewModel
    participant Sound as SoundPlayer
    participant Reader as ICardReader
    participant Dialog as CardTypeSelectionDialog
    participant StaffDlg as StaffManageDialog
    participant CardDlg as CardManageDialog

    Note over VM: 未登録のIDmを検出

    VM->>Sound: Play(SoundType.Warning)
    VM->>Reader: ReadBalanceAsync(idm)
    Reader-->>VM: 残高（事前読み取り）
    VM->>Reader: ReadHistoryAsync(idm)
    Reader-->>VM: 履歴（事前読み取り）

    VM->>Dialog: カード種別選択ダイアログを表示
    Note over Dialog: 「職員証」か「交通系ICカード」かを<br/>ユーザーに選択させる<br/>（IDmからの自動判別は技術的に不可能）
    User->>Dialog: 種別を選択

    alt 職員証を選択
        Dialog-->>VM: StaffCard
        VM->>StaffDlg: InitializeWithIdm(idm)
        Note over StaffDlg: 職員管理画面で新規登録
    else 交通系ICカードを選択
        Dialog-->>VM: IcCard
        VM->>CardDlg: InitializeWithIdmBalanceAndHistory(idm, balance, history)
        Note over CardDlg: カード管理画面で新規登録<br/>（カード種別はユーザーが手動選択）
    else キャンセル
        Dialog-->>VM: Cancel
    end

    VM->>VM: ResetState()
```

---

## 8. タイムアウト処理シーケンス

```mermaid
sequenceDiagram
    autonumber
    participant User as ユーザー
    participant VM as MainViewModel
    participant Timer as DispatcherTimer

    Note over User,Timer: 状態: WaitingForStaffCard

    User->>VM: 職員証をタッチ

    VM->>VM: 状態を WaitingForIcCard に変更
    VM->>Timer: Start(60秒)
    VM->>VM: RemainingSeconds = 60

    loop 毎秒
        Timer->>VM: Tick イベント
        VM->>VM: RemainingSeconds--
        VM->>VM: 画面表示更新（残り XX 秒）

        alt RemainingSeconds == 0
            VM->>Timer: Stop()
            VM->>VM: 状態を WaitingForStaffCard に戻す
            VM->>VM: 画面表示更新
            Note over VM: タイムアウト: 職員証タッチ待ちに戻る
        end
    end

    alt ユーザーが交通系ICカードをタッチ
        Timer->>Timer: Stop()
        Note over VM: 正常処理: 貸出/返却を実行
    end

    alt ユーザーがEscキーを押下
        VM->>Timer: Stop()
        VM->>VM: 状態を WaitingForStaffCard に戻す
        Note over VM: キャンセル: 職員証タッチ待ちに戻る
    end
```

---

## 9. エラー処理シーケンス

### 9.1 TransactionScope の自動ロールバック仕様（重要）

`DbContext.BeginTransactionAsync()` が返す `TransactionScope` は RAII 的なラッパーであり、**`using` を抜けた時点で `scope.Commit()` も `scope.Rollback()` も呼ばれていなければ `Dispose()` が自動的にロールバックする**。

このため、例外経路での明示的な `Rollback()` 呼び出しは**不要**である（呼んでも冗長になるだけ）。

- 正常系: `scope.Commit()` → トランザクション確定
- 例外系: 例外が `using` 文を突き抜ける → `scope.Dispose()` が自動ロールバック
- 明示ロールバック: `scope.Rollback()` は "明示的にロールバックしたい" ことを意図的に示す場合のみ使用（可読性向上）

詳細は [`05_クラス設計書.md` §5.5b 接続リース・トランザクションスコープ](05_クラス設計書.md#55b-接続リーストランザクションスコープissue-1243) を参照。

### 9.2 エラー処理シーケンス図

```mermaid
sequenceDiagram
    autonumber
    participant VM as MainViewModel
    participant LS as LendingService
    participant DB as DbContext
    participant Sound as ISoundPlayer

    VM->>LS: LendAsync(staffIdm, cardIdm)

    LS->>DB: await BeginTransactionAsync()
    DB-->>LS: TransactionScope (using)

    LS->>LS: 処理実行中...

    alt 正常系
        LS->>DB: scope.Commit()
        Note over LS,DB: using 脱出時に Dispose() が<br/>トランザクション→リースの順で解放
        LS-->>VM: LendingResult(Success = true)
    else データベースエラー発生
        Note over LS: 例外が using を突き抜ける
        Note over LS,DB: scope.Dispose() が<br/>自動ロールバック（Rollback 不要）
        LS-->>VM: LendingResult(Success = false, ErrorMessage)
    end

    VM->>Sound: Play(SoundType.Error)
    VM->>VM: エラーメッセージを表示
    VM->>VM: 背景色を薄い赤に変更

    Note over VM: 3秒後に自動的に通常状態に戻る

    VM->>VM: 状態を WaitingForStaffCard に戻す
    VM->>VM: 画面表示更新
```

> **SQLITE_BUSY / SQLITE_LOCKED 時の自動リトライ**: 書き込み系では `await _dbContext.ExecuteWithRetryAsync(async () => { ... })` で上記トランザクションブロックを包むのが推奨形（共有モードでは最大 5 回、指数バックオフ + ジッター付き）。本図では簡略化のため省略している。

---

## 10. 履歴統合シーケンス

```mermaid
sequenceDiagram
    autonumber
    participant User as ユーザー
    participant VM as MainViewModel
    participant Auth as StaffAuthService
    participant MS as LedgerMergeService
    participant LR as ILedgerRepository
    participant SG as SummaryGenerator
    participant OL as OperationLogger
    participant DB as DbContext

    User->>VM: 2件以上の履歴を選択して統合ボタンクリック

    VM->>VM: 統合条件チェック
    Note right of VM: ・同一カードか<br/>・貸出中レコードなし<br/>・受入/払出混在なし

    alt 条件不適合
        VM-->>User: エラーメッセージ表示
    end

    VM->>Auth: RequestAuthenticationAsync("履歴の統合")
    Auth-->>VM: StaffAuthResult

    alt 認証失敗
        VM-->>User: キャンセル
    end

    VM->>MS: MergeAsync(ledgerIds, operatorIdm)

    MS->>LR: GetByIdAsync(各ledgerId)
    LR->>DB: SELECT FROM ledger
    DB-->>LR: Ledger一覧
    LR-->>MS: List~Ledger~

    MS->>MS: バリデーション(Validate)
    Note right of MS: VM側の統合条件チェックと同じ3条件に加え<br/>チャージとポイント還元の混在を拒否（Issue #1736）<br/>混在集合はGenerateが空文字を返し<br/>摘要が空欄の行が保存されるため

    alt バリデーション不適合
        MS-->>VM: LedgerMergeResult(エラー)
        VM-->>User: エラーメッセージ表示
    end

    MS->>MS: 最古のLedgerを統合先に決定
    MS->>MS: 全Detailを統合先に移動
    MS->>MS: Income/Expense合算
    MS->>SG: Generate(allDetails)
    SG-->>MS: 再生成された摘要
    Note right of MS: Issue #1736: 空文字が返った場合は<br/>統合先の元の摘要を維持（空欄で保存しない）

    MS->>DB: await BeginTransactionAsync()
    DB-->>MS: TransactionScope (using)
    MS->>LR: MergeLedgersAsync(統合先Id, 統合元Id群, 統合先Ledger, tx)
    Note over LR,DB: 明細移動(UPDATE) → 統合先更新(UPDATE) → 統合元削除(DELETE)<br/>Issue #1753: 統合先UPDATEと統合元DELETEの影響行数を検証

    alt 影響行数が想定どおり（競合なし）
        LR-->>MS: true
        MS->>OL: LogLedgerMergeAsync(before, after, tx)
        MS->>DB: scope.Commit()
        Note right of MS: Issue #1954: ここで Success=true を確定させる<br/>（以降の失敗は成否に巻き込まない）
        MS->>LR: SaveMergeHistoryAsync(undoData)
        Note right of MS: Undoデータは別系統のため独立した tx
        alt Undoデータの保存に成功
            MS-->>VM: LedgerMergeResult(Success)
            VM-->>User: 統合完了メッセージ（「元に戻す」で取り消せます）
        else Undoデータの保存が失敗（SQLITE_BUSY / UNC 断）
            Note right of MS: Issue #1954: catch (ex) when (result.Success) で吸収<br/>Success は落とさず HasPostCommitFailure=true
            MS-->>VM: LedgerMergeResult(Success, HasPostCommitFailure)
            VM-->>User: 「統合は完了・取り消し情報を記録できませんでした」<br/>（再実行は促さない。Issue #1725）
        end
    else 影響行数が 0（他PCが先に統合・削除）
        LR-->>MS: false
        MS->>DB: scope.Dispose() で自動ロールバック
        MS-->>VM: LedgerMergeResult(競合エラー)
        VM->>VM: LoadHistoryLedgersAsync()
        Note right of VM: Issue #1753: 失敗時も一覧を再読込する<br/>（文言で「画面を更新して再試行」と案内するため）
        VM-->>User: 「対象の履歴が他の操作で変更された可能性があります…」
    end

    Note over MS,DB: using 脱出時に scope.Dispose() が<br/>トランザクションとリースを順次解放
```

> **競合検出は影響行数で行う（Issue #1753）**: 読み取り（`GetByIdAsync`）と書き込み（`MergeLedgersAsync`）は別トランザクションのため、共有モードではその間に他 PC が同じ履歴を統合・削除し得る。行ロックを持たないため、書き込み時の影響行数で競合を検出する（楽観ロック）。詳細と適用範囲は `05_クラス設計書.md` §5.5b「更新・削除は影響行数で競合を検出する」を参照。

---

## 11. 履歴分割シーケンス

```mermaid
sequenceDiagram
    autonumber
    participant User as ユーザー
    participant VM as LedgerDetailViewModel
    participant Auth as StaffAuthService
    participant SS as LedgerSplitService
    participant LR as ILedgerRepository
    participant SG as SummaryGenerator
    participant OL as OperationLogger
    participant DB as DbContext

    User->>VM: 明細をグループ分けして分割ボタンクリック

    VM->>VM: 2グループ以上か確認

    VM->>Auth: RequestAuthenticationAsync("履歴分割")
    Auth-->>VM: StaffAuthResult

    VM->>SS: SplitAsync(ledgerId, groupedDetails, operatorIdm)

    SS->>LR: GetByIdAsync(ledgerId)
    LR->>DB: SELECT FROM ledger
    DB-->>LR: Ledger
    LR-->>SS: Ledger（元の履歴）

    Note over SS: グループ1: 元のLedgerを更新
    SS->>SS: CalculateGroupFinancials(group1)
    SS->>SG: Generate(group1Details)
    SG-->>SS: 摘要
    SS->>LR: ReplaceDetailsAsync(元のLedger, group1Details)
    SS->>LR: UpdateAsync(元のLedger)

    loop グループ2〜N
        SS->>SS: CalculateGroupFinancials(groupN)
        SS->>SG: Generate(groupNDetails)
        SG-->>SS: 摘要
        SS->>LR: InsertAsync(新規Ledger)
        SS->>LR: InsertDetailsAsync(details)
    end

    SS->>OL: LogLedgerSplitAsync(operatorIdm, ...)

    SS-->>VM: LedgerSplitResult(Success)
    VM-->>User: 分割完了メッセージ
```

---

## 12. 職員認証シーケンス

```mermaid
sequenceDiagram
    autonumber
    participant VM as 各ViewModel
    participant Auth as StaffAuthService
    participant Dialog as StaffAuthDialog
    participant Reader as ICardReader
    participant SR as IStaffRepository
    participant Sound as ISoundPlayer

    VM->>Auth: RequestAuthenticationAsync("操作の説明")

    Auth->>Auth: App.IsAuthenticationActive = true
    Auth->>Dialog: Show(operationDescription)

    Note over Dialog: 60秒タイムアウト開始

    loop カード待ち受け
        Reader->>Dialog: CardRead イベント(IDm)
        Dialog->>SR: GetByIdmAsync(idm)
        SR-->>Dialog: Staff?

        alt 未登録の職員証
            Dialog->>Sound: Play(SoundType.Error)
            Dialog->>Dialog: エラーメッセージ表示、再度待ち受け
        else 登録済み職員証
            Dialog->>Dialog: 認証成功 → ダイアログ閉じる
        end
    end

    alt タイムアウト（60秒経過）
        Dialog->>Sound: Play(SoundType.Warning)
        Dialog->>Dialog: ダイアログ閉じる
        Auth->>Auth: App.IsAuthenticationActive = false
        Auth-->>VM: null（認証失敗）
    else 認証成功
        Auth->>Auth: App.IsAuthenticationActive = false
        Auth-->>VM: StaffAuthResult(Idm, StaffName)
    end
```

---

## 13. バックアップ/リストアシーケンス

```mermaid
sequenceDiagram
    autonumber
    participant User as ユーザー
    participant VM as SystemManageViewModel
    participant BS as BackupService
    participant SR as ISettingsRepository
    participant DB as DbContext

    Note over User,DB: バックアップ実行

    User->>VM: バックアップボタンクリック
    VM->>BS: ExecuteAutoBackupAsync()

    BS->>SR: GetAppSettingsAsync()
    SR-->>BS: AppSettings(BackupPath)

    alt BackupPath未設定
        BS->>BS: デフォルトパスを使用
    else パス検証
        BS->>BS: PathValidator.ValidateBackupPathAsync()
        Note over BS: 検証全体を Task.Run でオフロード（Issue #1746）。<br/>同期版は UNC 到達性チェック（最大5秒）が<br/>UI スレッドをブロックするため使わない
        alt 無効なパス
            BS->>BS: デフォルトパスにフォールバック
        end
    end

    BS->>BS: Directory.CreateDirectory(backupPath)
    BS->>BS: ファイル名生成（backup_yyyyMMdd_HHmmss.db）

    BS->>BS: 一時ファイルへ SQLite Backup API でコピー<br/>（backup_….db.{8桁の識別子}.tmp）
    Note over BS: Issue #1748: 最終名へ直接書くと、コピー途中で失敗した<br/>「マジックヘッダだけ揃った切り詰めファイル」が<br/>有効な世代として残る。ランダムな識別子は共有モードで<br/>複数台が同一の一時ファイルを開くのを防ぐ

    alt コピー成功
        BS->>BS: File.Move（一時ファイル → 最終名）
        Note right of BS: 切り替え点はメタデータ操作1回。<br/>「最終名で存在する＝コピー完了」が成立する
    else コピー失敗
        BS->>BS: 一時ファイルを削除（失敗は Warning ログのみ）
        Note right of BS: catch 内の後始末が例外を投げると<br/>本来の失敗要因を置き換えて抜けてしまう
    end

    BS->>BS: 一時ファイル回収（24時間以上前のもの）
    Note right of BS: プロセス強制終了で残った残骸のみ。<br/>他PCが書き込み中のものを消さないためのしきい値
    BS->>BS: 古いバックアップ削除（保持ルール超過分）
    Note right of BS: 世代の列挙は EnumerateBackupFiles に集約。<br/>一時ファイルは backup_*.db に一致しないため<br/>世代数を消費しない<br/>Issue #1813: 保持は「ファイル数」ではなく<br/>自動＝直近30日分（各日1世代）／<br/>手動・リストア前＝最新10件

    BS-->>VM: バックアップファイルパス
    VM-->>User: 完了メッセージ

    Note over User,DB: リストア実行

    User->>VM: リストアボタンクリック（バックアップファイル選択済み）
    VM->>BS: RestoreFromBackupAsync(backupFilePath)
    Note right of BS: Issue #1809: Task.Run で sync 版 RestoreFromBackup を<br/>バックグラウンドへ委譲（SuspendConnections が<br/>セマフォを同期取得するため UI スレッドでは<br/>#1281 のガードで拒否される）

    BS->>BS: バックアップファイル存在確認
    BS->>BS: IsValidSqliteFile（マジックヘッダ＋切り詰め検出）
    Note right of BS: Issue #1748: ヘッダが宣言するサイズ<br/>（ページサイズ×ページ数）と実ファイルサイズを照合し、<br/>本修正以前に作られた破損世代を弾く。<br/>判定はヘッダ100バイトの読み取りのみ<br/>（当時 UI スレッド上で走っていたため全長読み取りは不可。<br/>#1809 で Task.Run 先へ移った後も軽量判定を維持）
    alt 無効なファイル
        BS-->>VM: false（本番 DB は変更しない）
    end
    BS->>DB: SuspendConnections()
    Note right of DB: 接続セマフォを取得し進行中の<br/>トランザクション・リースの完了を待ってから<br/>SQLite接続を閉じてファイルロックを解除（Issue #1809）。<br/>停止中に到着した書き込みは解除まで待機

    BS->>BS: 現在のDBを.tempに退避
    BS->>BS: File.Copy(バックアップ → DBパス)

    alt コピー成功
        BS->>BS: .tempファイル削除
        BS-->>VM: true
    else コピー失敗
        BS->>BS: .tempから元のDBを復元
        BS-->>VM: false
    end

    VM-->>User: 結果メッセージ
```

---

## 14. CSV入出力シーケンス

```mermaid
sequenceDiagram
    autonumber
    participant User as ユーザー
    participant VM as DataExportImportViewModel
    participant Exp as CsvExportService
    participant Imp as CsvImportService
    participant SR as IStaffRepository
    participant CR as ICardRepository
    participant LR as ILedgerRepository

    Note over User,LR: CSVエクスポート

    User->>VM: エクスポートボタンクリック（種別・ファイルパス選択済み）

    alt 職員データ
        VM->>Exp: ExportStaffAsync(filePath, includeDeleted)
        Exp->>SR: GetAllAsync(includeDeleted)
        SR-->>Exp: List~Staff~
        Exp->>Exp: CSV書き出し
        Exp-->>VM: 出力件数
    else カードデータ
        VM->>Exp: ExportCardsAsync(filePath, includeDeleted)
        Exp->>CR: GetAllAsync(includeDeleted)
        CR-->>Exp: List~IcCard~
        Exp->>Exp: CSV書き出し
        Exp-->>VM: 出力件数
    else 履歴データ
        VM->>Exp: ExportLedgersAsync(filePath, startDate, endDate)
        Exp->>LR: GetByDateRangeAsync(startDate, endDate)
        LR-->>Exp: List~Ledger~
        Exp->>Exp: CSV書き出し
        Exp-->>VM: 出力件数
    end

    VM-->>User: 完了メッセージ（X件出力）

    Note over User,LR: CSVインポート

    User->>VM: インポートボタンクリック（ファイル選択済み）

    VM->>Imp: PreviewStaffImportAsync(filePath)
    Imp->>Imp: CSVパース → バリデーション
    Imp-->>VM: List~ImportPreviewItem~

    VM-->>User: プレビュー表示

    User->>VM: インポート実行

    VM->>Imp: ImportStaffAsync(filePath, skipExisting)
    Imp->>SR: InsertAsync / UpdateAsync
    Imp-->>VM: ImportResult(inserted, updated, skipped, errors)

    VM-->>User: 結果メッセージ
```

---

## 15. 印刷プレビューシーケンス

```mermaid
sequenceDiagram
    autonumber
    participant User as ユーザー
    participant RVM as ReportViewModel
    participant PVM as PrintPreviewViewModel
    participant RDB as ReportDataBuilder
    participant PS as PrintService
    participant CR as ICardRepository
    participant LR as ILedgerRepository

    User->>RVM: プレビューボタンクリック

    RVM->>RDB: BuildAsync(cardIdm, year, month)
    RDB->>CR: GetByIdmAsync(cardIdm)
    CR-->>RDB: IcCard
    RDB->>LR: GetByMonthAsync(cardIdm, year, month)
    LR-->>RDB: List~Ledger~
    RDB->>RDB: 繰越・月計・累計を計算
    RDB-->>RVM: MonthlyReportData

    RVM->>PS: CreateFlowDocument(data, orientation)
    PS-->>RVM: FlowDocument

    RVM->>PVM: SetDocument(data, title)
    PVM->>PVM: Document = flowDocument
    PVM->>PVM: ページ数計算

    PVM-->>User: プレビュー画面表示

    alt ズーム操作
        User->>PVM: ZoomIn / ZoomOut / ResetZoom
        PVM->>PVM: ZoomLevel変更 → ContentScale更新
    end

    alt ページナビゲーション
        User->>PVM: 次ページ / 前ページ / 最初 / 最後
        PVM->>PVM: CurrentPage更新
    end

    alt 印刷方向変更
        User->>PVM: 縦横切替
        PVM->>PS: CreateFlowDocument(data, newOrientation)
        PS-->>PVM: 新FlowDocument
        PVM->>PVM: Document差替え → 1ページ目にリセット
    end

    alt 印刷実行
        User->>PVM: 印刷ボタン
        PVM->>PS: PrintWithSettings(title, orientation, BuildDocument)
        PS->>PS: RequestPrint(orientation)（印刷ダイアログ）
        alt 印刷する
            PS->>PVM: BuildDocument(印刷ダイアログの用紙寸法)
            PVM->>PS: CreateFlowDocument(data, pageSize)
            PS-->>PVM: 印刷用の新FlowDocument（改ページ位置を再計算）
            PVM-->>PS: 印刷用FlowDocument
            PS->>PS: プリンターへ送信（プレビューのDocumentは書き換えない、Issue #2047）
        end
        PS-->>PVM: true/false
    end
```

---

## 16. 管理者ダッシュボードシーケンス(Issue #1692)

```mermaid
sequenceDiagram
    participant User as 管理者
    participant Dlg as AdminDashboardDialog
    participant VM as AdminDashboardViewModel
    participant Svc as AdminDashboardService
    participant Repo as Repository群
    participant RES as ReportExportStatusService
    participant Xls as AdminDashboardExcelExportService

    User->>Dlg: F8 で画面を開く
    Dlg->>VM: LoadOperationStatusAsync()

    Note over Svc,Repo: SQLite は同一接続での並列クエリを許さないため直列 await(Issue #1452)
    VM->>Svc: GetOperationStatusAsync(asOf, しきい値)
    Svc->>Repo: 設定 → カード → 貸出中レコード → 最新残高 → 最終利用日(#1747) → 職員
    Repo-->>Svc: 各結果
    Note over Svc,RES: 出力先フォルダのファイル走査は同期処理のため Task.Run へオフロード
    Svc->>RES: GetStatuses(対象カード, 出力先, 年, 月)
    RES-->>Svc: 出力済み / 未出力 / 判定不可
    Svc-->>VM: AdminDashboardOperationStatus
    VM->>VM: 絞り込み適用・要約文をステータスへ

    Note over User,VM: 利用分析は開いた時点では集計しない(台帳は 6 年分)
    User->>Dlg: 稼働状況 / 利用推移タブを選択
    Dlg->>VM: EnsureAnalyticsLoadedAsync()

    alt 未読み込みのとき
        VM->>Svc: GetAnalyticsAsync(from, to, asOf)
        Svc->>Repo: 利用実績 / 月別職員別 / 月末残高 を SQL 側で GROUP BY
        Repo-->>Svc: 集計済みの行
        Svc-->>VM: AdminDashboardAnalytics
        VM->>VM: ChartScale / ChartGeometryCalculator で座標を算出
    else 読み込み済み
        VM-->>Dlg: 何もしない
    end

    alt Excel 出力
        User->>VM: Excel出力ボタン → 保存先を選択
        VM->>Xls: ExportAsync(運用状況, 利用分析, パス)
        Note over Xls: 分析が未取得なら分析系 3 シートは作らない<br/>(空シートは「0 件だった」と誤読される)
        Xls-->>VM: 完了
    end
```

---

## 付録A. 関連ドキュメント

| ドキュメント | 関連箇所 |
|---|---|
| `04_機能設計書.md` | 各シーケンスの業務仕様・状態遷移の背景 |
| `05_クラス設計書.md` | 登場するクラス(`LendingService`, `LedgerMergeService`, `StaffAuthService` 等)の責務 |
| `02_DB設計書.md` | シーケンス中で参照される SQL / テーブル |
| `07_テスト設計書.md` | 各シーケンスに対応するテストケース |
