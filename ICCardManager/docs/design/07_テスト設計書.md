...
| 種別 | テスト数 | 備考 |
|------|---------|------|
| 単体テスト（ICCardManager.Tests） | 2,577件 | xUnit + FluentAssertions + Moq |
...
| No | テストケース | 入力 | 期待結果 |
|----|-------------|------|---------|
| 1 | 残高<閾値の判定 | 999/1000/1001/0, 閾値1000 | 999と0のみ警告対象（< 比較） |
...
| 2-4 | <5秒経過 | 0/2/4秒前 | "たった今" |
| 5-8 | 5-59秒経過 | 5/15/30/59秒前 | "N秒前" |
| 9-11 | 60秒以上経過 | 60/120/3599秒前 | "N分前" |
| 12-15 | IsStale境界(15秒) | 0/14/15/60秒前 | <15: false, ≥15: true |
| 16 | RecordRefresh直後 | 呼び出し後 | "たった今", IsStale=false |
| 17 | Start で2タイマー生成 | Start() | ヘルスチェック(30秒) + 同期表示(1秒) |
| 18 | Stop で全停止 | Start→Stop | 全タイマー IsRunning=false |
| 19 | Start 2回目で再生成 | Start→Start | 1回目のタイマーは停止、新しい2つが追加 |
| 20 | OnSyncDisplayTick発火 | SimulateTick() | SyncDisplayUpdated呼出 |
| 21 | 排他制御フラグ | IsHealthCheckRunning | 初期false, SetHealthCheckRunningで反映 |
...
| 3 | **3件鉄道のIncome/Expense/Balance** | 100+150+200円 | Income=0, Expense=450, Balance=最新Detail |
