...
| No | テストケース | 入力 | 期待結果 |
|----|-------------|------|---------|
| 1 | スタンドアロン時のカード一覧キャッシュ | IsSharedMode = false | TTL = 30秒 |
| 2 | 共有モード時のカード一覧キャッシュ | IsSharedMode = true | TTL = 5秒 |
| 3 | 共有モード時の貸出中カードキャッシュ | IsSharedMode = true | TTL = 3秒 |
| 4 | 共有モード時の職員一覧キャッシュ | IsSharedMode = true | TTL = 10秒 |
| 5 | 共有モード時の設定キャッシュ | IsSharedMode = true | TTL = 1分 |
...
| 2 | 貸出後のキャッシュ反映 | PC-Aでカード1を貸出 → PC-Bで貸出中一覧を確認 | キャッシュTTL（3秒）経過後にPC-Bにも反映 |
