...
> **用語定義:**
> - **暗黙還元（暗黙のポイント還元）**: ICカード履歴上、IsPointRedemption=falseだが、Amount（金額）が負数であるエントリ。チャージでもなく明示的なポイント還元フラグもないが、金額が負であることからポイント還元と判定する。
> - **明示還元（明示的ポイント還元）**: ICカード履歴上、IsPointRedemption=trueが明示的にセットされているエントリ。
