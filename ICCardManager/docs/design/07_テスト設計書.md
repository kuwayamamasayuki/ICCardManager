...
| 種別 | テスト数 | 備考 |
|------|---------|------|
| 単体テスト（ICCardManager.Tests） | 2,491件 | xUnit + FluentAssertions + Moq |
...
#### UT-051: 同期表示テキスト・stale判定・ライフサイクル

**設計ポイント:** `ISystemClock` を注入して時刻を固定化。`DateTime.Now` 直接使用と
リフレクション依存を排除してフレーク性を完全に除去している。

| No | テストケース | 入力 | 期待結果 |
|----|-------------|------|---------|
| 1 | 最終同期なし | _lastRefreshTime=null | "同期待ち...", IsStale=false |
| 2-4 | <5秒経過 | 0/2/4秒前 | "最終同期: たった今" |
| 5-8 | 5-59秒経過 | 5/15/30/59秒前 | "最終同期: N秒前" |
| 9-11 | 60秒以上経過 | 60/120/3599秒前 | "最終同期: N分前" |
| 12-15 | IsStale境界(15秒) | 0/14/15/60秒前 | <15: false, ≥15: true |
| 16 | RecordRefresh直後 | 呼び出し後 | "たった今", IsStale=false |
| 17 | RecordRefresh後の経過 | Refresh→時計20秒進める | "20秒前", IsStale=true |
| 18 | Start で2タイマー生成 | Start() | ヘルスチェック(30秒) + 同期表示(1秒) |
| 19 | Stop で全停止 | Start→Stop | 全タイマー IsRunning=false |
| 20 | Start 2回目で再生成 | Start→Start | 1回目のタイマーは停止、新しい2つが追加 |
| 21 | OnSyncDisplayTick発火 | SimulateTick() | SyncDisplayUpdated呼出 |
