...
| 5 | 太字赤文字ハイライト | RichTextセルに変更フィールド | 変更値が太字+赤文字 |
| 6 | 取り消し線 | strikethrough=true | 全ランに取り消し線 |
| 7 | NULL JSON書き込み | null JSONをセルに書き込み | セル空 |
| 8 | UPDATE行のExcel出力 | UPDATE操作をエクスポート | 変更後列に太字+赤文字のラン |
| 9 | DELETE行のExcel出力 | DELETE操作をエクスポート | 変更前列に取り消し線 |
| 10 | INSERT行のExcel出力 | INSERT操作をエクスポート | ハイライトなし |
