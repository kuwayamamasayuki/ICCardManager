...
#### UT-039: ICカード待ち状態でのエラー（職員証タッチ）

| No | テストケース | 操作 | 期待結果 |
|----|-------------|------|---------|
| 1 | エラー音 | ICカード待ちで別の職員証タッチ | Play(SoundType.Error)呼出 |
| 2 | 状態不変 | ICカード待ちで職員証タッチ | CurrentState=WaitingForIcCard（変化なし） |
| 3 | 警告通知 | ICカード待ちで職員証タッチ | ShowWarning("職員証です", "交通系ICカードをタッチしてください") |
...
### 2.36 ICカードリーダー再接続ロジック（PcScCardReader, Issue #1168）

#### UT-048: 自動再接続フェーズ管理

| No | テストケース | 操作 | 期待結果 |
|----|-------------|------|---------|
| 1 | 高速リトライ間隔定数 | ReconnectIntervalMs | 3000 (3秒) |
| 2 | 高速リトライ最大回数 | MaxReconnectAttempts | 10 |
| 3 | 低頻度リトライ間隔定数 (#1168) | SlowReconnectIntervalMs | 60000 (60秒) |
| 4 | 上限到達後もタイマー継続 (#1168) | 11回試行→上限超過 | タイマー稼働継続、Interval=60000 |
