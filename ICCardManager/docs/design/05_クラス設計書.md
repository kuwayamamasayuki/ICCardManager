...
| BackupService | データベースのバックアップ（SQLite Backup API使用。共有モード時も安全に実行可能）。UI スレッドから呼ぶ場合は `CreateBackupAsync` を使用すること（sync 版 `CreateBackup` は `DbContext.LeaseConnection()` の UI スレッドガード (Issue #1281) に抵触する。Issue #1361） |
