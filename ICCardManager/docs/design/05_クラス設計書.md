...
- 接続の正常性確認と定期ヘルスチェックは **`DbContext` ではなく `SharedModeMonitor`**（上記クラス一覧参照）が担う。共有フォルダモード時のみ **15秒間隔**（`SharedModeMonitor.HealthCheckIntervalSeconds = 15`、Issue #1493）でタイマーを起動（`Start()` → `ExecuteHealthCheckAsync()`）し、軽量クエリ（`SELECT COUNT(*) FROM sqlite_master`、Issue #1110: 実ファイルアクセスを強制し切断を検出）で接続を確認、接続断時は自動再接続を試行する。`DbContext` 自身はヘルスチェック用のメソッド（旧記載の `CheckConnectionHealth()` / `StartHealthCheck()`）を持たない
