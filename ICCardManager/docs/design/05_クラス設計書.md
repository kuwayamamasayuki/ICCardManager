...
- **ファイル到達確認の上限時間**（Issue #1716）: 到達不能な UNC への `File.Exists` は下位の TCP/SMB タイムアウトまで戻らない（実測 21〜42 秒、直後の再呼び出しは 0 秒）。別スレッドで実行し `AppConstants.DatabaseReachabilityProbeTimeoutSeconds`（5秒。`ReachabilityProbeTimeout` で仮想化しテストから短縮可能）以内に完了しなければ「到達不能」とみなす。上限で打ち切っても `File.Exists` は中断できないため、**進行中の確認は 1 本に限定**し（`_pendingReachabilityProbe`）、ブロック済みスレッドが 15 秒ごとに積み上がるのを防ぐ。到達失敗は `LogInformation` で記録する（`appsettings.json` の既定フィルタが Information のため `LogDebug` は本番のログファイルに残らない）。詳細は 04_機能設計書 §18.4
