...
### 1.3 ICカード待ち中の職員証タッチ（持ち替え対応 / Issue #1211）

**背景:** 運用上、ICカードリーダー上に職員証を置きっぱなしにする職員がおり、他の職員が操作しようとすると置きっぱなしの職員証が先に反応してしまうことがある。このとき操作者（B）が自分の職員証を改めてタッチし直せるよう、ICカード待ち状態での職員証タッチを「持ち替え」として扱い、以降の貸出/返却はタッチし直した職員の名義で記録する。

| ステップ | 処理内容 |
|----------|----------|
| 1 | ICカード待ち状態で職員証IDmを受信 |
...
**挙動上の注意:**
- ICカード待ち状態中に読み取られた職員証は、**別職員の持ち替え / 同一職員の再タッチを問わず、初回タッチと完全に同じ動作**（Notify 音 + 認識トースト + 操作者(上書き)設定 + タイムアウトリセット）として処理する。
- これにより、持ち替え3回目以降（A→B→C）でも毎回同じフィードバックが得られ、運用者は最後にタッチされた職員名を常にトースト上で確認できる。
- 操作者が切り替わっても ledger の `lender_idm` / `staff_name` は必ず最後にタッチされた職員のものになる。
...
> **2層ロック構造（共有フォルダモード）**: 共有フォルダモードでは、アプリケーション層の `CardLockManager`（プロセス内排他制御）に加えて、SQLiteの `busy_timeout`（ファイルレベル排他制御）による2層のロック構造で同時アクセスを制御する。CardLockManagerは同一プロセス内のスレッド間排他を担い、busy_timeoutは異なるPC間のDB書き込み競合を担う。
...
> **データベースパス設定**: `C:\ProgramData\ICCardManager\database_config.txt` でDBファイルの配置先を指定する。settingsテーブルではなくファイルで管理する（DB接続前に読み込む必要があるため）。インストーラーの「データベースの保存先」ページまたは設定画面（F5）から変更可能で、変更後はアプリケーション再起動が必要。
...
複数PCから同一のSQLiteデータベースにアクセスする運用形態。SMB共有フォルダ上にDBファイルを配置し、`C:\ProgramData\ICCardManager\database_config.txt` にUNCパスを設定することで有効化される。インストーラーの「データベースの保存先」ページで「共有フォルダで複数PCから使用」を選択してUNCパスを入力するか、設定画面（F5）から変更できる。

### 17.2 共有モード判定

`database_config.txt` にパスが設定されている場合、アプリケーションは共有フォルダモードとして動作する。`DbContext.IsSharedMode` プロパティで判定可能。

### 17.3 キャッシュTTLの調整

共有フォルダモードでは、他PCによるデータ変更を早期に反映するため、キャッシュのTTL（Time To Live）を短縮する。ローカル操作ではキャッシュが即座に無効化されるため、TTLは「他PCの操作結果が見えるまでの遅延」のみに影響する。20台同時接続でのネットワーク負荷を考慮し、過度に短くしない。
...
共有フォルダモードでは、30秒ごとにDB接続のヘルスチェックを実行する。
