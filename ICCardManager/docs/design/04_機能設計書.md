...
> **データベースパス設定**: `appsettings.json` の `DatabaseOptions.Path` でDBファイルの配置先を指定する。settingsテーブルではなくappsettings.jsonで管理する（DB接続前に読み込む必要があるため）。設定UIからも変更可能で、変更後はアプリケーション再起動が必要。
...
複数PCから同一のSQLiteデータベースにアクセスする運用形態。SMB共有フォルダ上にDBファイルを配置し、`appsettings.json` の `DatabaseOptions.Path` にUNCパスを設定することで有効化される。

### 17.2 共有モード判定

`DatabaseOptions.Path` が空でない場合、アプリケーションは共有フォルダモードとして動作する。`DbContext.IsSharedMode` プロパティで判定可能。
