...
| 3 | 受信IDmが現在の操作者と異なる（別職員の持ち替え）場合のみ、通常の職員証認識トースト（「〇〇 さん / 交通系ICカードをタッチしてください」）を再表示 |
| 4 | 状態は `WaitingForIcCard` を維持し、タイムアウトをリセット |
| 5 | 続いて交通系ICカードがタッチされた場合、**(上書き後の)職員情報で貸出/返却が記録される** |

**挙動上の注意:**
- 音（Notify / Error 等）は一切鳴らさない。置きっぱなしの職員証で毎回音が鳴るのを避けるため。
- 同一職員の職員証が再度読み取られた（同じ人のカードが連続反応した）だけの場合は、トースト通知も出さない（完全無動作）。
- 別職員に切り替わった場合のみ、通常の貸出開始時と同じトーストを新しい職員名で表示する。これにより、操作者は「どちらの名義で次の処理が走るか」をトースト上の氏名で目視確認できる。
