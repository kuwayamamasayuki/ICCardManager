...
| 3 | 状態は `WaitingForIcCard` を維持し、タイムアウトをリセット |
| 4 | 続いて交通系ICカードがタッチされた場合、**(上書き後の)職員情報で貸出/返却が記録される** |

**挙動上の注意:** ICカード待ち中に職員証がタッチされた場合、音・トースト通知は一切出さない（別職員でも同一職員でも同じ扱い）。これは置きっぱなしの職員証や持ち替えが日常的に発生するため、操作ノイズを抑制することを優先した判断による。
