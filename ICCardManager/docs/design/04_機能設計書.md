...
| 2 | 受信IDmが現在の操作者IDmと同一 → 操作ノイズとして静かに無視（タイムアウトのみリセット） |
| 3 | 受信IDmが現在の操作者IDmと異なる → 操作者情報（IDm / 氏名）を新しい職員に上書き |
| 4 | 職員証認識音（Notify）を再生 |
| 5 | 「〇〇さんを認識しました」トースト通知を表示 |
| 6 | 状態は `WaitingForIcCard` を維持し、タイムアウトをリセット |
| 7 | 続いて交通系ICカードがタッチされた場合、**上書き後の職員情報で貸出/返却が記録される** |

**注意:** この動作は、同一職員による連続タッチ（ステップ2）ではエラー音・警告を出さないため、置きっぱなし運用でノイズが出ないようになっている。
