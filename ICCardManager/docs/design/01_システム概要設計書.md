...
> **共有フォルダモード**: `appsettings.json` の `DatabaseOptions.Path` にUNCパス（例: `\\server\share\iccard.db`）を指定することで、複数PCから同一データベースにアクセスできる。最大約20台を想定し、SQLiteのロック機構とリトライ処理で同時アクセスを制御する。
