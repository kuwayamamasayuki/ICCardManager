...
`before_data` と `after_data` は、対象テーブルの該当行を表す **平坦なキーバリューJSONオブジェクト** として保存する。`target_table` のカラム名をキーとし、値はDB上の表現（文字列・数値・null）をそのまま埋め込む。INSERT 時の `before_data` と DELETE 時の `after_data` は `null` とする。
...
シリアライズには `System.Text.Json` を使用し、`null` フィールドも省略せず出力する（差分比較を容易にするため）。
...
スキーマ変更は `src/ICCardManager/Data/Migrations/` の連番マイグレーションクラスとして実装されており、`MigrationRunner` がアプリ起動時に未適用分を順次実行する。
...
> 新規マイグレーションを追加した際は、本表およびテーブル定義の該当カラム説明に「マイグレーション番号 / Issue番号」を必ず記載すること。
...
`C:\ProgramData\ICCardManager\database_config.txt` にUNCパスを指定することで、SMB共有フォルダ上のデータベースファイルを使用できる（共有フォルダモード）。
...
| 項目 | 説明 |
|------|------|
| 設定方法 | インストーラーの「データベースの保存先」ページまたは設定画面（F5）から設定。設定値は `C:\ProgramData\ICCardManager\database_config.txt` に保存される |
...
## 9. 同時アクセス制御（共有フォルダモード）

### 9.1 ロック戦略

共有フォルダモードでは、複数PCからSQLiteデータベースに同時アクセスが発生する。SQLiteのファイルレベルロックとアプリケーション層のリトライ機構で排他制御を行う。
...
SQLiteのWAL（Write-Ahead Logging）モードはネットワーク共有上のデータベースでは正しく動作しない。WALモードは共有メモリ（`-shm`ファイル）を使用するが、SMB/CIFS経由ではメモリマップドファイルの一貫性が保証されないため、データ破損のリスクがある。そのため、DELETEジャーナルモードを明示的に指定する。DELETEが設定できない環境（一部のNAS等）では、TRUNCATE → PERSISTの順にフォールバックする。OFFモード（クラッシュ耐性なし）は使用しない。

### 9.2 リトライ機構

`SQLITE_BUSY`（データベースがロック中）および`SQLITE_LOCKED`（テーブルがロック中）エラー発生時に、指数バックオフでリトライを行う。共有モードではリトライ回数と待機時間を増加し、ジッター（ランダム遅延）を付加してthundering herd問題を緩和する。
...
共有フォルダモードでは、ネットワーク切断によりDB接続が失われる可能性がある。定期的にヘルスチェックを実行し、接続が失われた場合は自動再接続を試みる。
...
### 9.4 物理ICカードによる自然な排他制御

本システムでは、物理的なICカードをPaSoRiにタッチして操作を行う。同一の物理ICカードを同時に2台のPCでタッチすることは物理的に不可能であるため、同一カードに対する同時書き込みは原理的に発生しない。このため、楽観的ロック（バージョン管理等）のような追加のアプリケーションレベル排他制御は不要である。
