...
| 項目 | 設定値 | 説明 |
|------|--------|------|
| busy_timeout | 5000ms | ロック待機の最大時間。他PCがDBをロック中の場合、最大5秒待機する |
| journal_mode | DELETE | ジャーナルモード。ネットワーク共有ではWALモードが動作しないため、DELETEモードを明示的に指定 |

#### WALモードを使用しない理由

SQLiteのWAL（Write-Ahead Logging）モードはネットワーク共有上のデータベースでは正しく動作しない。WALモードは共有メモリ（`-shm`ファイル）を使用するが、SMB/CIFS経由ではメモリマップドファイルの一貫性が保証されないため、データ破損のリスクがある。そのため、DELETEジャーナルモードを明示的に指定する。

### 9.2 リトライ機構

`SQLITE_BUSY`（データベースがロック中）および`SQLITE_LOCKED`（テーブルがロック中）エラー発生時に、指数バックオフでリトライを行う。
