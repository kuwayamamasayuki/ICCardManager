...
#### 2.5.11 `MessageBox` にはオーナーウィンドウを渡す（#1794）
...
- `IDialogService` 経由の表示は `DialogService` 内の private `Show` へ集約済みで、そこがオーナーを解決する。**`MessageBox.Show` の直呼びを `DialogService` へ増やさないこと**（`DialogServiceOwnerTests` の静的検査が検出する）
...
- **移行途上**: `MainViewModel` / `ReportViewModel` / `ErrorDialogHelper` / 各 `Window` コードビハインドに `MessageBox.Show` の直呼びが約 34 か所残っている。**Issue #1837** が段階的な移行を追跡する。新規コードでは直呼びせず `IDialogService` を使うこと

詳細規約は `.claude/rules/development-conventions.md`（UI/UX原則）、設計は `docs/design/05_クラス設計書.md` §5.18a を参照。
