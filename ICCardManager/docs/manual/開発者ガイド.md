...
UI テスト基盤は**マニュアル用スクリーンショットの自動撮影**にも使う（`ManualScreenshotTests`）。Windows ネイティブの PowerShell から `tools\take-screenshots-uitest.ps1` を実行すると、本体を Release でビルド・起動し、サンプルデータを投入した状態でメイン画面・履歴画面・各ダイアログを `docs\screenshots\auto\` へ保存する。撮影は環境変数 `ICCARDMANAGER_SCREENSHOT=1` のときだけ動き、通常のテスト実行では Skip される。詳細は `docs/screenshots/README.md` を参照。
