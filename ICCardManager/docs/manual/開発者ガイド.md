...
| パス長 (≤260) | Windowsの制限 |
| 絶対パス | 相対パスは危険 |
| UNCパス | 共有フォルダモード時のみ許可（`DatabaseOptions.Path` 経由） |
| パストラバーサル禁止 | `..` を含むパス禁止 |
| 不正文字チェック | `<>"|` 等 |
...
`appsettings.json` の `DatabaseOptions.Path` にUNCパスを指定する。空欄の場合はローカルのデフォルトパス（`C:\ProgramData\ICCardManager\iccard.db`）が使用される。

```json
{
  "DatabaseOptions": {
    "Path": "\\\\server\\ICCardShare\\iccard.db"
  }
}
```

設定画面（F5）からも変更可能。変更後はアプリケーションの再起動が必要。
...
| ファイル | 場所 | 備考 |
|----------|------|------|
| iccard.db | %PROGRAMDATA%\ICCardManager\ | 全ユーザー共有（デフォルト）。`DatabaseOptions.Path` でUNCパス指定時は共有フォルダ上に配置 |
