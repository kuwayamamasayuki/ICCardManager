...
- オーナー選択の優先順位は **アクティブ → モーダル子に塞がれていない（`IsEnabled`）ウィンドウのうち最後 → 使用可能な最後**。**`Application.MainWindow` を選ばない**（モーダルが開いている間にメイン画面をオーナーにすると、そのダイアログが無効化されず欠陥が残る）
- 候補は「表示済み・ハンドルあり・**アクティブ化する**（`ShowActivated`）」ものに限る。`ShowActivated="False"` の `ToastNotificationWindow` は**貸出・返却のたびに最後に生成される**ため、外さないと非フォアグラウンド時にトーストがオーナーに選ばれ、①業務画面が有効なまま残る②トーストの自動クローズ（3 秒）で表示中のダイアログごと破棄される、という害が出る。**新しく非モーダルの通知系ウィンドウを足すときは `ShowActivated` を必ず明示すること**
