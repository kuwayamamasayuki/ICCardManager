...
ビルド警告ゼロを維持する。CI（`ci.yml` の code-quality ジョブ）が Release ビルドの警告を数え、1 件でもあれば fail する。ローカルでも変更時に Release / Debug 双方でソリューションビルドし 0 警告を実測すること（CI は Release のみ検証する）。
...
| UI テスト | 実際に起動したアプリの画面操作（`tests/ICCardManager.UITests`） | xUnit + FlaUI 5.0。GUI が必要なため CI では実行しない |
