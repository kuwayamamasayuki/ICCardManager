...
graph LR
    ...
    subgraph "ハードウェア"
        PASORI[PaSoRi<br>RC-S300/RC-S380]
...
> **NuGet ロックファイル**: `<RestorePackagesWithLockFile>true</RestorePackagesWithLockFile>` を有効化し、`packages.lock.json` で推移的依存のバージョンを固定している（サプライチェーン攻撃対策）。CI は `dotnet restore --locked-mode` で復元するため、ロックファイルと食い違うと NU1004 で失敗する。パッケージを更新したら（Dependabot の PR を含む）、手元で **Windows の** `dotnet restore --force-evaluate` を実行して**ソリューション内のすべての** `packages.lock.json` の変更をコミットすること — 本体のパッケージを更新すると、本体を参照するテスト・DebugDataViewer のロックファイルも変わる（Dependabot は更新したプロジェクトのロックファイルしか書き換えない）。Windows で実行するのは、本体と DebugDataViewer のロックファイルにある RID の節（`.NETFramework,Version=v4.8/win7-x86`）が Windows の SDK でしか生成されないため — Linux（WSL の `dotnet`・Dependabot）で作ると節が落ち、それだけで NU1004 になる。ロックファイルの中身は SDK の版でも変わる（SDK 10 は RID を `win-x86` と書き、暗黙の `Microsoft.NETFramework.ReferenceAssemblies` 参照を書かない）ため、リポジトリ直下の `global.json` で SDK を 8.0 系に固定している。開発機には .NET 8 SDK（8.0.400 以上）が必要。同じ理由で、SDK がマシンの環境（.NET Framework の targeting pack の有無）によって暗黙に足す `Microsoft.NETFramework.ReferenceAssemblies` は `Directory.Build.targets` で明示している。GitHub Actions のアクションはタグではなく commit SHA で固定している（Issue #2117）。
...
│       │   └── Migrations/              # DB マイグレーション（#001〜#009。スキーマ構築の Single Source）
...
> **refactor 履歴（v2.5.0〜v2.8.0）**: `MainViewModel` からのサービス抽出（#1222）、`CsvImportService` の partial class 分割（#1223）と再分割（#1284）、`LedgerRepository` のインターフェース階層分離（#1224）、`ISystemClock` 導入によるテスト容易化（#1228）、Value Object 化（#1221）、`LendingService.LendAsync` / `ReturnAsync` の internal ヘルパー分割（#1283）、Migration 冪等化（#1285）、Service 層への `ConfigureAwait(false)` 一貫適用（#1287）により、責務分離とテスト容易性の向上が継続している。詳細は §2.5 「アーキテクチャの発展」を参照。
...
### 2.5 アーキテクチャの発展（v2.5.0〜v2.8.0）

v2.5.0〜v2.8.0 で導入された主要な構造変更は、いずれも **「テスト容易性」** と **「責務分離」** の向上を目的としている。新規開発者はこの節で現在のコード構造の背景を理解してほしい。
...
`GetConnection()` を非推奨（`[Obsolete]`）化し（#1209）、以下のスコープベース API へ統一した。**生の `SQLiteConnection` を返す API は Issue #1988 で削除済み**で、接続はリース／スコープ経由でのみ取得できる。
...
> **1 メソッドで 2 文以上（DELETE→INSERT 等）を発行するリポジトリメソッドを書くときは、必ず 3 分岐にする**（Issue #1724）。SQLite は接続に活性トランザクションが無いと各文を autocommit で即確定するため、`command.Transaction` を設定し忘れた文は後続文が失敗しても巻き戻らない。また外側スコープ内で自前 `BeginTransactionAsync()` を開くとセマフォ再取得で自己デッドロックする（Issue #1575）。分岐の一覧と実装例は `docs/design/05_クラス設計書.md` §5.5b を参照。

#### 2.5.7 LendingService の internal ヘルパー分割（#1283、v2.8.0）

長大化していた `LendingService.LendAsync`（121 行）と `ReturnAsync`（182 行）を、責務単位の `internal` ヘルパーメソッドへ分割し、可読性とテスト容易性を向上させた（それぞれ 62 行・99 行に短縮）。

| 抽出先ヘルパー | 役割 |
|----------------|------|
| `ValidateLendPreconditionsAsync` / `ValidateReturnPreconditionsAsync` | 貸出・返却の事前条件検証 |
| `ResolveLentRecordAsync` | 既存の貸出中レコード解決 |
| `ResolveInitialBalanceAsync` / `ResolveReturnBalanceAsync` | 残高解決 |
| `InsertLendLedgerAsync` / `PersistReturnAsync` | DB 永続化 |
| `FilterUsageSinceLent` | 貸出後に発生した利用履歴の抽出 |
| `ApplyBalanceWarningAsync` | 残高警告の適用 |

`public` API は一切変更していないため、既存テストは全件 pass。新規に `LendingServiceHelperTests`（23 件）を追加し、各ヘルパーを個別に検証している。

#### 2.5.8 CsvImportService の再分割（#1284、v2.8.0）

§2.5.3 の partial class 分割（#1223）に続く第 2 段階。`Ledger.cs`（1,031 行 → 520 行）と `Detail.cs`（1,042 行 → 761 行）を以下の責務へさらに分離した。

- `LedgerCsvRowParser` — Import/Preview 間で重複していた利用履歴行パース（約 200 行）を共通化
- `LedgerDetailCsvRowParser` — 利用履歴詳細の 13 列パース
- `NewLedgerFromSegmentsBuilder` — Detail の「履歴 ID 空欄 → 新規 `Ledger` 自動作成」ロジック（Issue #906/#918/#1053 関連）
- `CsvImportService.LedgerValidation.cs` — 検証系ヘルパー 4 メソッドを別 partial へ分離

`public` API は変更せず、既存 94 件のテストは全件 pass。抽出クラス向けに単体テスト 21 件（`LedgerCsvRowParserTests` / `LedgerDetailCsvRowParserTests` / `NewLedgerFromSegmentsBuilderTests`）を追加。

#### 2.5.9 Migration の冪等化（#1285、v2.8.0）

共有モードで複数 PC が同時に初回マイグレーションを試みた場合や、`schema_migrations` テーブルが部分破損した状態で再起動した場合に、`ALTER TABLE ADD COLUMN` 系マイグレーションが「column already exists」で例外停止していた問題を解消した。

- `MigrationHelpers.AddColumnIfNotExists(conn, tx, table, column, "TYPE DEFAULT ...")` を新設
- 非冪等だった 5 マイグレーション（#002 / #003 / #005 / #006 / #009）を冪等化
- 全 9 マイグレーションの二重実行テスト `MigrationIdempotencyTests`（9 件）と `MigrationHelpers` 単体テスト（7 件）を追加
- 冪等パターンの規約を `.claude/rules/migrations.md` に文書化

> 新規マイグレーションを追加する際は `.claude/rules/migrations.md` の「冪等パターン」セクションに従うこと。`CREATE TABLE IF NOT EXISTS` / `CREATE INDEX IF NOT EXISTS` / `INSERT OR IGNORE` / `MigrationHelpers.AddColumnIfNotExists` の 4 種を組み合わせる。

#### 2.5.10 Service 層への `ConfigureAwait(false)` 一貫適用（#1287、v2.8.0）

WPF UI スレッドへの不要な継続 dispatch を排除し、性能向上とデッドロック予防を図るため、Service 層の `await` 全てに `.ConfigureAwait(false)` を付与する規約を導入した。

| 層 | 方針 |
|----|------|
| Services/ | `ConfigureAwait(false)` を**必ず**付与 |
| ViewModels/ | 付け**ない**（`INotifyPropertyChanged` や WPF バインディングが UI 文脈を要求） |
| Views/ | 付けない |
| Tests | 付けない（regression detector としての純粋性を保つ） |

`.editorconfig` で `CA2007` を Service 層のみ `suggestion` レベル、ViewModels / Views / tests は `none` で無効化。詳細規約は `.claude/rules/async-configureawait.md` を参照。Phase 1（#1287）で 11 ファイル約 130 箇所を対応、Phase 2（#1350）で残り 11 ファイル約 94 箇所を対応済み。`DialogService` / `StaffAuthService` など UI 依存サービスは個別判断で除外。

#### 2.5.11 `MessageBox` にはオーナーウィンドウを渡す（#1794 / #1837）

`MessageBox.Show(message, title, button, image)` のようにオーナーを渡さないオーバーロードでは、WPF が `GetActiveWindow()` でオーナーを解決する。**呼び出しスレッドがフォアグラウンドでないときこれは NULL になり**（ownerless）、下位のウィンドウが無効化されない。処理中オーバーレイ（`IsBusy` にバインドされた全面 `Border`）は視覚要素であると同時に**全面クリックシールド**でもあったため、#1383 → #1784 → #1793 でオーバーレイを先に閉じる構造へ寄せたことにより、ダイアログ背後のボタンが押せる状態になった。

| 表示手段 | 影響 | 対処 |
|----------|------|------|
| `MessageBox.Show`（オーナー無し） | **あり**。下位ウィンドウが無効化されない | `Common/DialogOwnerResolver.Resolve()` の結果を渡す |
| `Window.ShowDialog()`（`INavigationService.ShowDialog<T>`） | なし。WPF がアプリ内の他ウィンドウをすべて無効化する | 対処不要（`Owner` は z 順・タスクバー表示にのみ影響） |

**表示手段は次の 3 つに限る（#1837）。これ以外は `MessageBoxOwnerConventionTests` がリポジトリ全体の静的検査で検出する。**

| 呼び出し元 | 手段 |
|---|---|
| `Window` のコードビハインド | `MessageBox.Show(this, …)`（自ウィンドウが正しいオーナー） |
| ViewModel | `IDialogService` 経由（`BusyScope` の静的検査の対象にもなり、単体テストからモックで差し替えられる） |
| `App` の起動失敗・`ErrorDialogHelper` の致命エラーなど、`Application.Current` が無い状況で呼ばれ得る層 | `Common/OwnedMessageBox` |

- **ownerless フォールバック（オーナーを解決できないときオーナー無しで表示する分岐）は `OwnedMessageBox` の 1 か所だけ**。経路ごとに書き写すと、次に規約を変える人が一部を取りこぼす（`Common/SafeRollback` が #1831 で同じ判断をした）
- `IDialogService` 経由の表示は `DialogService` 内の private `Show` へ集約済みで、そこがオーナーを解決して `OwnedMessageBox` へ委譲する。**`MessageBox.Show` の直呼びを `DialogService` へ増やさないこと**（`DialogServiceOwnerTests` の静的検査が検出する）
- 3 択（はい／いいえ／キャンセル）が要る場面は `IDialogService.ShowThreeWayConfirmation`（戻り値 `bool?`。はい=true／いいえ=false／中止=null）を使う。**「いいえ」が処理を進める選択肢**（帳票を別名で保存する等）のときだけ使い、2 択で足りるなら `ShowConfirmation` にする
- オーナー選択の優先順位は **アクティブ → モーダル子に塞がれていない（Win32 `IsWindowEnabled`）ウィンドウのうち最後 → 使用可能な最後**。**`Application.MainWindow` を選ばない**（モーダルが開いている間にメイン画面をオーナーにすると、そのダイアログが無効化されず欠陥が残る）。「塞がれているか」は WPF の `Window.IsEnabled` では分からない（`ShowDialog()` は Win32 の `EnableWindow` で無効化し、`IsEnabled` は変わらない）
- WPF 以外のウィンドウ（表示中の `MessageBox` / `OpenFileDialog` 等）がアクティブなときは従来どおり WPF の解決（`GetActiveWindow()`）に委ねる。入れ子の `MessageBox` が外側の `MessageBox` を無効化する従来挙動を保つため
- 候補は「ハンドルあり（表示済みでクローズ前）・**アクティブ化する**（`ShowActivated`）」ものに限る（`IsVisible` は条件に含めない — メイン画面の最小化で隠れた所有ダイアログを候補に残すため）。`ShowActivated="False"` の `ToastNotificationWindow` は**貸出・返却のたびに最後に生成される**ため、外さないと非フォアグラウンド時にトーストがオーナーに選ばれ、①業務画面が有効なまま残る②トーストの自動クローズ（3 秒）で表示中のダイアログごと破棄される、という害が出る。**新しく非モーダルの通知系ウィンドウを足すときは `ShowActivated` を必ず明示すること**
- **移行は Issue #1837 で完了した**（#1794 時点で残っていた約 34 か所）。マルチモニタでの `Window.ShowDialog()` の配置（`Owner` が `Application.Current.MainWindow` 固定）は本欠陥とは別問題で、未対応のまま

詳細規約は `.claude/rules/ui-conventions.md`（UI/UX原則）、設計は `docs/design/05_クラス設計書.md` §5.18a・§5.18b を参照。
...
#### 適用記録の書き込みも冪等であること（Issue #1738）

`Up()` を冪等にしても、適用記録の書き込みが素の `INSERT` なら共有モードの同時起動で後発 PC が起動不能になる。`MigrateTo` は冒頭で `GetCurrentVersion()` を 1 回読むだけで、そのスナップショットは他 PC の適用で陳腐化する。加えて `ApplyMigration` のトランザクションは `BEGIN IMMEDIATE` のため、後発 PC は先発 PC の COMMIT を待ってから同じ version を INSERT することになり、busy_timeout の待機がむしろ衝突を確定させる。

`schema_migrations` へ INSERT する箇所は `MigrationRunner.RecordMigration` と `DbContext.BackfillLegacyMigrationVersion1` の 2 か所で、いずれも `INSERT OR IGNORE` を使う。`OR IGNORE` は PRIMARY KEY 衝突以外の制約違反（`NOT NULL` / `CHECK`）も握りつぶすため、`RecordMigration` は影響行数が 0 のときに行の有無を読み戻し、「他 PC が先に記録した（正常）」と「書き込めなかった（異常）」を区別する。後者を見逃すと、記録が入らないまま `Up()` が毎起動で再適用され続ける。
...
### 4.7 ビルド警告の扱い（Issue #1786）

ビルド警告ゼロを維持する。CI（`ci.yml` の code-quality ジョブ）が Release と Debug の両方のビルドで警告を数え、1 件でもあれば fail する（Issue #2099 までは Release のみだった）。ローカルでも変更時に Release / Debug 双方でソリューションビルドし 0 警告を実測すること。

```bash
# WSL2 の場合は dotnet を "/mnt/c/Program Files/dotnet/dotnet.exe" に読み替える
dotnet build ICCardManager/ICCardManager.sln -c Release
dotnet build ICCardManager/ICCardManager.sln -c Debug
```

- 警告は原則として**宣言側を是正**して消す。`NoWarn` による抑制も選択肢だが、その場合は同じ csproj のコメントに警告 ID と理由を必ず併記する
- `CS8618`（未初期化の非 Null 許容フィールド）は抑制しない。テストフィクスチャの初期化漏れという実バグを示し得るため
- `= null!` は「必ず非 null」という宣言であり、実際に null チェックしているフィールドに付けると意図とコードが食い違う。null になり得るなら `?` を付ける
- 上記は `BuildWarningSuppressionConventionTests` が csproj の静的検査で固定している。詳細規約は `.claude/rules/development-conventions.md` の「ビルド警告は『抑制』ではなく『是正』で消す」を参照
...
| 処理 | 対象 |
|------|------|
| 不正なサロゲート除去 | ペアになっていない上位・下位サロゲート |
| 制御文字除去 | \x00-\x08, \x0B, \x0C, \x0E-\x1F, \x7F-\x9F（タブ・改行・復帰は残す） |
| ゼロ幅文字除去 | \u200B-\u200D, \u2060, \uFEFF |
| 前後空白除去 | 全角・半角スペース |
| 内部空白正規化 | 連続スペースを単一に |
| 最大長での切り詰め | 職員名 50・職員番号 20・備考 200・管理番号 20 文字。サロゲートペア（絵文字など）を途中で切らない |
...
- ディレクトリ（`C:\ProgramData\ICCardManager`、`backup`、`Logs`）の権限は**インストーラー** (`ICCardManager.iss`) が `Permissions: users-full` で設定する
- 複数の職員（Windowsユーザー）が同一PCで利用するため、ディレクトリレベルで全ユーザーにアクセスを許可
- ランタイム（C# 側）では権限を変更しない。`Directory.CreateDirectory` のみ呼び出し、ACL の設定はインストーラーに委ねる
- ファイルはディレクトリからの**権限継承**により保護（ファイルに明示的ACLは設定しない）
- SQLite関連ファイル（-wal, -shm, -journal）も同様に継承を有効化

> **Issue #1455**: 旧実装ではランタイムで `BUILTIN\Users : FullControl` を `AddAccessRule` で付与していた。これは
> (1) `FullControl` が削除権限まで含むため、一般ユーザーが他ユーザーのバックアップを削除・差替え可能となる過剰権限であり、
> (2) `AddAccessRule` が冪等でないため起動の度に ACE が累積する、という二重の問題があった。インストーラーが既に `users-full` を設定済みのため、ランタイムでの再付与は撤廃した。

```csharp
// ディレクトリ: 作成のみ。ACL の設定はインストーラー (ICCardManager.iss) が users-full を付与済み。
// 共有モード（UNC パス）等インストーラーの管理外パスを使う場合の権限は管理者責任とする。
// Issue #1499: 旧名は EnsureDirectoryWithPermissions。Issue #1455 で ACL 操作を撤廃した結果、
// 実体が Directory.CreateDirectory の薄いラッパーとなったため、命名と挙動の乖離を解消するためにリネームした。
...
共有モードでは15秒ごとにデータベースへの接続を確認し（`SharedModeMonitor.HealthCheckIntervalSeconds = 15`、Issue #1493）、接続が失われた場合は自動的に再接続を試行する。ネットワーク切断時はステータスバーに警告が表示される。

#### VACUUM制約

VACUUMは月初（月の10日以降の初回起動時）に、`SettingsRepository.TryAcquireMonthlyVacuumLockAsync` の先勝ち CAS ロック（`settings.last_vacuum_date` のアトミック更新）で1台のみが自動実行する（Issue #1482）。共有モードでは他のPCが接続中の場合にVACUUMが失敗する可能性があるが、ロック獲得後の失敗は当月スキップとして確定し、翌月の月初まで再試行しない（失敗のたびに再試行してロックを奪い合うデッドロックスパイラルの防止）。実装は `App.xaml.cs` の起動時タスク参照。
...
| UI テスト | 実際に起動したアプリの画面操作（`tests/ICCardManager.UITests`） | xUnit + FlaUI 5.0。GUI が必要なテスト（`Category=UI`）は CI では実行しない。同じプロジェクトにある GUI 不要のテスト（Category を持たないもの）は、CI が csproj を直接指定して実行する（UITests はソリューション単位の `dotnet test` から自分を外しているため。Issue #2099） |

UI テスト基盤は**マニュアル用スクリーンショットの自動撮影**にも使う（`ManualScreenshotTests` / `TouchScreenshotTests`）。Windows ネイティブの PowerShell から `tools\take-screenshots-uitest.ps1` を実行すると、本体を Release と Debug でビルドし、サンプルデータを投入した状態でメイン画面・履歴画面・各ダイアログ（Release）と、仮想タッチによる職員証認識・貸出・返却・トースト（Debug。撮影モード `ICCARDMANAGER_SCREENSHOT_MODE=1` で操作パネルを透明にしテストデータ自動登録を止める）を `docs\screenshots\auto\` へ保存する。撮影は環境変数 `ICCARDMANAGER_SCREENSHOT=1` のときだけ動き、通常のテスト実行では Skip される。詳細は `docs/screenshots/README.md` を参照。

Debug パスでは、職員証認証（`StaffAuthDialog`）を挟む画面も撮る。履歴行の追加・貸出記録の作成は監査対象の操作で職員証のタッチを要求するため、実カードリーダーの無い撮影環境では DEBUG 限定の「職員証仮想タッチ」でしか通せない。カード側のタッチは仮想タッチダイアログから行う。

![仮想タッチダイアログ](../screenshots/virtual_touch.png){width=50%}

*▲ カード・職員・利用履歴を指定して、実カードなしで貸出・返却を再現する（DEBUG ビルド限定、Issue #1577）。*

一方、**投入データで作れない外部状態に依存する画像は自動化の対象外**である。カードリーダー未接続（`error_no_reader.png`）は撮影機に PaSoRi が繋がっていると撮れないため切断時だけ撮る Skip 条件を持ち、共有モードのネットワーク切断（`warning_network_disconnected.png`）と OS 標準の保存ダイアログを経る手動バックアップ（`backup_completed_status.png`）は対話式の `tools\TakeScreenshots.ps1` に残している。

画面を構成するソース（XAML・ViewModel・共通スタイル）を変えたときは、`-Changed` を付けると対応表 `docs/screenshots/screenshot-sources.json` と git の差分から**影響を受けた画像だけ**を撮り直せる（`-Changed -Publish` はその画像だけを差し替える）。判定は `tools\screenshot-sync.ps1` が行い、Claude Code の Stop フックと CI（`screenshot-sync-check.yml`）も同じ判定で「画面ソースが変わったのに画像が未更新」を案内する。撮影対象を増やしたら対応表にも 1 行足すこと（単体テスト `Tools/ScreenshotSyncMappingTests` が撮影テストとの一致を固定している）。
...
```powershell
# マニュアル用スクリーンショットの自動撮影（Windows ネイティブ PowerShell）
.\tools\take-screenshots-uitest.ps1            # docs\screenshots\auto\ へ出力
.\tools\take-screenshots-uitest.ps1 -Publish   # 撮影せず、auto\ の画像を見比べたうえで docs\screenshots\ へ上書き（対応表に載っている画像だけ）
.\tools\take-screenshots-uitest.ps1 -Changed            # 画面ソースの変更（origin/main との差分）で影響を受けた画像だけ撮影
.\tools\take-screenshots-uitest.ps1 -Changed -Publish   # その画像だけ上書き
.\tools\screenshot-sync.ps1 -Verify                     # 撮らずに「影響を受けたのに未更新の画像」を列挙（終了コード 3）
```
...
| テスト失敗（マイグレーション関連） | `schema_migrations` 状態の競合や Up() の非冪等性 | `MigrationIdempotencyTests` を参照点に `:memory:` SQLite で個別検証。マイグレーション規約は `.claude/rules/migrations.md` |
| テスト失敗（UI スレッド関連） | Service 層から UI スレッドの DbContext 操作を直接呼んでいる | `DbContextUiThreadGuardTests` を参照点に該当呼び出しを `Task.Run` でオフロード（Issue #1281） |
| デバッグ時にカード認識しない | MockCardReaderが使用されている | 正常動作（デバッグモード） |
| 本番でカード認識しない | PaSoRiドライバ未インストール | ドライバをインストール |

**テスト隔離の補足**: 現代のテスト基盤では各テストクラスが xUnit の `IDisposable` パターンで `:memory:` SQLite 接続をコンストラクタで生成し `Dispose` で破棄するため、グローバル状態をクリアする手動メソッド（旧 `ClearOperationLogs()` 等）は存在しません。テスト間の状態共有は `xUnit.IClassFixture<T>` の明示的フィクスチャでのみ発生します（Issue #1491 でこの節を v2.8.0 のテスト基盤に合わせて更新）。
