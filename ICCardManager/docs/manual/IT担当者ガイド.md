...
管理者から「繰越情報が失われたカード」の一覧（管理者マニュアル §3.4）を渡されたときの修復対象です。値は `ic_card` テーブルの次の 4 列に入っています。

| 一覧の項目 | 列名 |
|------------|------|
| 開始ページ番号 | `starting_page_number` |
| 繰越累計受入 | `carryover_income_total` |
| 繰越累計払出 | `carryover_expense_total` |
| 対象年度 | `carryover_fiscal_year` |

- **一覧に値が表示されている列だけ**を書き戻してください。「（消失なし）」の項目はそのカードでは失われておらず、書き換えると正しい値を壊します。
- 修復は全 PC でアプリケーションを終了してから行い、事前に手動バックアップを取得してください。
- 反映後、各 PC でアプリケーションを再起動すると起動時の警告が消えます。
...
> - **`{0}` `{1}` `{2}` の 3 つすべてを含めてください。** いずれかが欠けていると既定の書式で出力されます。年度ファイルは「1 枚のカードの 1 年度ぶん」の月シートを積み上げるため、カード種別・管理番号・年度のどれかが名前に現れないと**別のカードや別の年度が同じファイルへ書き込まれ、先に作成した月のシートが消えます**（管理番号はカード種別と組でしか一意になりません）。
> - **拡張子は `.xlsx` にしてください。** 拡張子が無い書式や `.csv` `.xls` `.xlsm` などを指定すると既定の書式で出力されます（`.xlsx` 以外では Excel ファイルとして正しく保存できず、帳票の作成そのものが失敗するか、拡張子と中身の形式が食い違ったファイルになるため）。
