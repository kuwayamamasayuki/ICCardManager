...
- **撮影テストがスキップされた画像は完了報告で分けて表示される**。xUnit の `Skip.If` はテスト成功扱いなので `dotnet test` の終了コードでは「撮れなかった」と「撮る必要が無かった」が区別できない。完了報告は「今回の実行で撮影 N 枚 / 対象 K 枚」と、撮れなかった画像名を並べる。`error_no_reader.png` のように**その環境では原理的に撮れない画像**が対象に入っている場合、`-Publish` はエラーで止まる（前回の画像をそのまま公開しないため）ので、撮れる環境で撮り直すこと
