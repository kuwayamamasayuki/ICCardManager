...
依存のため対象外。**（追記: Issue #1817 で解消済み。`PathValidator.CreateWriteProbeIoFailure`
へ切り出し、生の `ex.Message` は `ErrorDialogHelper.LogException` でログへ逃がすようにした。
本 spec は #1614 制定より前の判断であり、以後この行を「既知残件」として参照しないこと。）**`"ネットワーク共有に到達できません..."`（行 145）は既に
3 要素を満たしているため対象外。
