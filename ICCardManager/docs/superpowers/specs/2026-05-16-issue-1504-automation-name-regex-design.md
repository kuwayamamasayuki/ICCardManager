...
新規テストメソッドを 1 件追加するため（`AutomationNamePattern_should_be_whitespace_tolerant_but_value_strict`、`[Theory]` 7 ケース）、`ICCardManager/docs/design/07_テスト設計書.md` の UT-058a に以下を反映する:

- 既存 No.5-14（キーコントロールの個別 Name 検査）の備考に「Issue #1504 以降は Regex マッチで XAML 整形の空白挿入を許容」を追記
- 新規 No.25-31 行として、合成 XAML 7 ケースの regression テストを追加

これは feedback memory「テスト追加・修正時はテスト設計書も同期更新する」に沿う対応。
