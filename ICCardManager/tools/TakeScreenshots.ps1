...
.DESCRIPTION
    ...
    メイン画面・履歴画面・ツールバーから開く 6 ダイアログ（計 8 枚）は
    tools\take-screenshots-uitest.ps1 で人の操作なしに撮影できます（Issue #2016）。
    本スクリプトは、タッチ操作や特殊な状態を要する残りの画面のために残しています。
...
$optionalScreens = @(
    ...
    # Issue #1410: ユーザーマニュアル §6.2 帳票作成完了後のファイル一覧
    # ReportDialog 内の「作成結果」GroupBox（CreatedFiles.Count > 0 で展開）を撮影する。
    # ListBox 内の Hyperlink でファイル名が青色リンクとして表示される視覚的手がかりが見えるよう、
    # 複数カード（2〜3 枚）を一括作成して 2 行以上表示された状態を撮影することが望ましい。
    @{
        Name = "report_files_list.png"
        Title = "帳票作成完了後のファイル一覧"
        Instructions = "F1キーで帳票作成画面を開き、対象月と複数カード（2〜3枚）を選択して「作成」ボタンをクリック。ダイアログ下部の「作成結果」セクションが展開され、ファイル名が青色リンクとして 2 行以上並んで表示されたら"
        ForegroundOnly = $true
    },
