...
namespace ICCardManager.Common.Charting
{
    ...
    internal static class ChartSeriesLabelDisambiguator
    {
        ...
        /// 一意にする。<b>ただし基底名そのものが「氏名（N 人目）」の形をしている場合は、
        /// 添えた結果が別の系列の基底名と一致し得る</b>（台帳の <c>staff_name</c> や職員マスタに
        /// その表記の氏名が実在する場合に限る）。追加の再判定は入れていない — 起こり得る条件が
        /// 極端に狭く、判定を重ねるほうがこの関数の読み解きを難しくするため。通し番号は表示順（＝利用額の降順）に依存するため期間を変えると
