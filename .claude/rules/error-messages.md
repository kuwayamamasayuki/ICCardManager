...
| `CsvImportService` の共通 catch（トランザクション**前**の失敗） | 無し | 変換関数にログを併設 |
| 同（トランザクション**内**の失敗） | 内側の catch が「ロールバックより先に」記録して再スロー（#1745） | **記録済みの印**を付け、共通ハンドラーでは記録しない |
| `CsvImportService.Detail.cs` の明細置換失敗 | 直前で `_logger?.LogError` 済み | 文言だけを組み立てる（ログは足さない） |

- **「呼び出し元が既に記録しているか」は、呼び出し元の一覧では数え切れない**。同じ公開メソッドでも、トランザクションの前で落ちたか中で落ちたかで答えが変わる。判定は**例外インスタンス自身**（`Exception.Data`）へ「記録済み」の印を置いて行う — サービスのフィールドに持つと、シングルトンのサービスで**並走した別のインポートが互いの印を見る**。ラップして投げ直す経路（`SQLiteException` → `DatabaseException`）では**新しい例外の側**へ印を付ける（外側の `catch` へ届くのは包んだ側）。再スローは必ず `throw;` で行う（`throw ex;` はスタックトレースを消す）
- **印を付けられないときは「未記録」へ倒す**。痕跡が残らないより二重に残るほうが障害調査では害が小さい
- **レベルは「システムの不具合か、利用者の入力の問題か」で分ける**（#1716）。ファイルが見つからない・権限が無い・文字コードが読めない（`AppException`）は職員が選び直せば解決する想定内の失敗で、その都度 `Error` を積むと本当の不具合が埋もれる。痕跡は残しつつ `Warning` にする
- **他の文へ埋め込むときは「なぜ」だけを埋める**（`ExceptionMessageFormatter.ToReason`）。`ToUserMessage` は「〇〇に失敗しました。」という「何が」と行動指示を含む**完全な文**なので、既に「何が」と「どうすれば」を述べている文へ埋め込むと、①「何が」が二重になって**矛盾し**（「明細は置き換えました」の直後に「明細の取り込みに失敗しました」）、②**両立しない行動指示**が並ぶ（「再度実行してください」と「履歴画面で修正してください」）。**行動指示を含む既製の文言**（`DatabaseException.QueryFailed` の「再度お試しください」）も同じ理由で埋め込まない
- **操作名は既定値を持たない引数で貫通させる**（#1956 / #1820）。既定値付きにすると全経路が同じ汎用名のままになり、「何が」が退化して、どの取り込みが失敗したのか文言から区別できなくなる
...
- **分岐が既に原因を名指ししているなら、`ToReason` を「詳細」として足さない**（Issue #1996 のコードレビューで検出）。`ToReason` は例外種別ごとの**定型文**なので、種別で分けた `catch` の中では**直前の文を別の言い方で繰り返すだけ**になり、「詳細」という語が空手形になる。さらに悪いことに、**継承関係で別の分岐に落ちて見出しと食い違う** — `DirectoryNotFoundException` は `IOException` の派生なので、「出力先フォルダが見つかりません」の直後に「ファイルの読み書き中に問題が発生しました」が続く。`ToReason` を併記してよいのは**種別が確定していない `catch (Exception)`** の側で、そこでは SQLite の Busy / Locked のように「なぜ」を実際に名指しする。生の `ex.Message` が運んでいた情報（パス・原文）はログにしか無くなるので、**文言はログへ誘導する**（「詳細はログファイルを確認してください。」）
- **具体性を約束するラベルを、定型文の前に置かない**（同）。`原因: {ToReason(ex)}` は「この後に原因が来る」と読ませるが、`Process.Start` の失敗（`Win32Exception` / `InvalidOperationException`）は必ず `default` 分岐の「予期しない問題が発生しました。」へ落ちる。**その分岐に実際に届く例外型を数えてから**ラベルを付ける。届く型が `default` にしか落ちないなら、ラベルを外して素の一文として並べる
...
- 回帰は `ErrorCodeUniquenessConventionTests` が静的検査で固定する。走査対象は
  **本番ソース全体**の `.cs` から**導出**する（ファイル名で列挙すると例外クラスが
  増えたときに静かに漏れる。#1786）。当初は `Common/Exceptions/` 配下に限っていたため、
  `ErrorDialogHelper` の `SYS00x` と `DuplicateCardNumberException` の `CARD001` が検査の外にあった
  — **走査範囲の境界も迂回経路になる**（Issue #2101）。重複は「ファイルをまたぐもの」ではなく
  **出現回数**で判定する（同じファイル内の 2 回目も衝突である）。手書きの一覧で一意性を検査しない
  （`AppExceptionTests` の一覧は `DB009` を載せ忘れていた）。「重複が無いこと」だけでなく**既知のコードが実際に拾えること**を
