...
- 回帰は `ErrorCodeUniquenessConventionTests` が静的検査で固定する。走査対象は
  ...
  **出現回数**で判定する（同じファイル内の 2 回目も衝突である）。走査範囲を広げた分、収集は
  **コードを割り当てる位置**（`ErrorCode` への代入・例外の生成と基底呼び出しの引数・`GetErrorInfo` のタプル）に限る —
  比較（`== "CR001"`）や `case "DB001":`、無関係なリテラル（`"UTF008"`）まで数えると誤検出になり、
  修正者をファイルの除外へ誘導する（#1786）。手書きの一覧で一意性を検査しない
...
| `CsvImportServiceLedgerTransactionTests.ImportLedgersAsync_SQLiteエラー_生の例外メッセージをUIへ出さないこと` | 履歴CSVインポートの書き込み失敗（Issue #1745）。`SQLiteException` を `DatabaseException.QueryFailed` へラップしてから再スローすることを、文言が `DatabaseException.QueryFailed().UserFriendlyMessage` と**完全一致**することで表明し、あわせて `NotContain("database is locked")` / `NotContain("予期しない")` で生の文言・既定分岐へ落ちていないことを表明。**否定の表明だけではラップを検証できない**（Issue #2105）— #1986 で `ToUserMessage` に SQLite 用の分岐が入ったため、ラップを外しても「データベースの読み書きができませんでした」という別の整備済み文言になり、否定の表明はすべて満たされる。既定分岐の文言も「予期しない**問題**」へ変わっており、`NotContain("予期しないエラー")` は原理的に失敗しなかった |
