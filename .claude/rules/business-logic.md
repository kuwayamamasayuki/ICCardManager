...
| 乗継 | 鉄道（A駅～C駅） ※途中駅省略。**乗継統合は貪欲に行うが、統合結果は 1 つに決め打たない**（Issue #1916）。往復の往路が直前チェーンの延長に消費されると往復表記が失われ、通しでは乗っていない区間（薬院→天神／天神→博多／博多→天神 で「薬院～博多」）が 6 年保存の台帳へ入るため、「延長を 1 箇所だけ抑止した候補」を往復検出まで通し、**往復として説明できた元区間（明細）の件数**で選び直す |
| 複数区間 | 鉄道（A駅～B駅、C駅～D駅） |
...
- **貪欲な統合の結果を、後段の判定にとっての最終解として扱わない**（Issue #1916）。乗継統合は往復判定より先に走るため、統合が往復の片割れを消費すると後段は往復を検出できない。#1902 は「復路が次の移動へ延長される」形を後方参照ガードで塞いだが、鏡像の「往路が直前チェーンの延長に消費される」形は残っていた。
  ...
  - **単一箇所の抑止しか試さないため、同じ形が 1 日に 2 回現れると片方しか是正されない**。しかも同点時のブロック数比較は「後半を是正した候補」を選び得るので、Issue 本文の形（先に現れる通し区間）が残ることがある
