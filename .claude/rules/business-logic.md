...
- **貪欲な統合の結果を、後段の判定にとっての最終解として扱わない**（Issue #1916）。乗継統合は往復判定より先に走るため、統合が往復の片割れを消費すると後段は往復を検出できない。#1902 は「復路が次の移動へ延長される」形を後方参照ガードで塞いだが、鏡像の「往路が直前チェーンの延長に消費される」形は残っていた。
  ...
  - **決められないなら決め打たず、候補を並べて後段の結果で選ぶ**。「延長を 1 箇所だけ抑止した候補」を往復検出まで通し、**往復として説明できた元区間（明細）の件数**で選ぶ。同点なら統合が進んでいる側（ブロック数が少ない側＝貪欲な既定解）を採り、従来挙動を保存する
  - **カバレッジは「統合後の本数」ではなく「元の区間数」で数える**。本数で数えると TC038 で往復 2 組へ分解した候補（4 本）が正解（2 本）を上回る
